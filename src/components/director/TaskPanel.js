...
import {
  ...
  ImageIcon, MessageSquare, Upload, ZoomIn,
} from "lucide-react";
...
function Modal({ title, onClose, children, maxWidth = "max-w-lg" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl w-full ${maxWidth} max-h-[92vh] overflow-y-auto z-10`}>
...
function ImageLightbox({ src, onClose }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <button onClick={onClose} className="absolute top-4 right-4 text-white/60 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-xl transition-colors">
        <X size={18} />
      </button>
      <img
        src={src}
...
function ImageUploadField({ value, onChange }) {
  ...
  return (
    <div>
      <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Proof / Image</label>
      {preview ? (
        <div className="relative rounded-xl overflow-hidden border border-gray-200 group">
          <img src={preview} alt="Preview" className="w-full max-h-48 object-cover" />
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            <span className="text-white text-xs font-semibold">Ready to upload</span>
          </div>
          <button type="button" onClick={clear} className="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white p-1 rounded-lg transition-colors shadow">
            <X size={12} />
          </button>
        </div>
      ) : (
        ...
        >
          <Upload size={20} className="mx-auto text-gray-300 group-hover:text-extra-blue transition-colors mb-2" />
          <p className="text-xs text-gray-400 group-hover:text-extra-blue transition-colors">Click or drag & drop an image</p>
  ...
  return (
    <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 mt-1">
      ...
      <div className="space-y-2">
        {items.map((r, i) => (
          <div key={i} className={items.length > 1 ? "border-l-2 border-amber-200 pl-2" : ""}>
            <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-wrap">{r.text || r}</p>
            {r.addedBy?.name && <p className="text-xs text-amber-400 mt-0.5">— {r.addedBy.name}</p>}
          </div>
        ))}
      </div>
    </div>
    ...
    <>
      <Modal title="Task Details" onClose={onClose}>
        <div className="space-y-4">
          ...
          {imageUrl && (
            <div className="rounded-xl overflow-hidden border border-gray-100 relative group cursor-pointer" onClick={() => setLightbox(true)}>
              <img src={imageUrl} alt="Task proof" className="w-full max-h-52 object-cover" />
          ...
          <button onClick={onClose} className="w-full border border-gray-200 text-gray-600 text-sm font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition-colors">
            Close
          </button>
        </div>
      </Modal>
      {lightbox && imageUrl && <ImageLightbox src={imageUrl} onClose={() => setLightbox(false)} />}
    </>
  );
}
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div>
        ...
        <input required value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="Brief task title"
      ...
      <div>
        ...
        <textarea rows={3} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Detailed instructions…"
      ...
      <div>
        ...
        <textarea rows={2} value={form.remarks} onChange={(e) => set("remarks", e.target.value)} placeholder="Add notes or updates..."
      ...
      <ImageUploadField value={form.image} onChange={(file) => set("image", file)} />
    ...
    <>
      <div className={`flex items-center gap-3 py-3 border-b border-gray-50 last:border-0 group ${done ? "opacity-60" : ""}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            <span className="text-xs text-gray-400 flex items-center gap-1">
              <User2 size={10} />{task.assignedTo?.name || "Unassigned"}
            </span>
            {task.raisedBy?.name && (
              <span className="text-xs text-extra-blue flex items-center gap-1 font-medium">· raised by {task.raisedBy.name}</span>
            )}
            {task.dueDate && (
            ...
            {remarksText && (
              <span className="text-xs text-amber-500 flex items-center gap-1">
                ...
                {remarksText.length > 40 ? remarksText.slice(0, 40) + "…" : remarksText}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={() => onView(task)} className="p-1.5 rounded-lg hover:bg-blue-50 text-gray-300 hover:text-blue-500 transition-colors" title="View details">
            <Eye size={12} />
          </button>
          <button onClick={() => onEdit(task)} className="p-1.5 rounded-lg hover:bg-amber-50 text-gray-300 hover:text-amber-500 transition-colors" title="Edit task">
            <Pencil size={12} />
          </button>
          <button onClick={() => onDelete(task)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-400 transition-colors" title="Delete task">
            <Trash2 size={12} />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-gray-50 transition-colors"
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-extra-darkblue rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
          </div>
          <span className="text-xs font-bold text-gray-400 w-8 text-right">{pct}%</span>
          {collapsed ? <ChevronRight size={14} className="text-gray-300" /> : <ChevronDown size={14} className="text-gray-300" />}
        </div>
      </div>
      {!collapsed && (
        <div className="px-5 pb-2 border-t border-gray-50">
          {tasks.length === 0
            ...
            : tasks.map((task) => (
                <ChecklistRow key={task._id} task={task} onStatusChange={onStatusChange}
...
export default function TaskPanel() {
  ...
  const handleUpdate = async (form) => {
    try {
      setActionLoading(true);
      await axiosInstance.patch(`/pending/update/${editTask._id}`, buildFormData(form));
      // ✅ No Content-Type header — axios sets it automatically with correct boundary
      setEditTask(null);
  ...
  const projectMap = {};
  projects.forEach((p) => { projectMap[p._id] = { name: p.name, tasks: [] }; });
  tasks.forEach((task) => {
    const pid = task.project?._id;
    if (pid && projectMap[pid]) {
      projectMap[pid].tasks.push(task);
    } else {
      projectMap["__none__"] = projectMap["__none__"] || { name: "No Project", tasks: [] };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={loadData} className="p-2 rounded-xl border border-gray-200 text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors" title="Refresh">
            <RefreshCw size={14} className={loading ? "animate-spin" : ""} />
          </button>
          <button onClick={() => { setDefaultProjectId(""); setShowCreate(true); }}
