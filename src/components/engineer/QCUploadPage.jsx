...
import { CheckSquare, Send, FileCheck, Image, Trash2, CheckCircle, Loader2, AlertCircle } from "lucide-react";
import {
  PageHeader, Card, SectionHead, Label, inputStyle, SubmitBtn,
  UploadBox, FONTS, PROJECTS as MOCK_PROJECTS, ITEMS_BY_PROJECT,
} from "./shared";
...
export default function QCUploadPage() {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      setProjects(Array.isArray(r.data) ? r.data : r.data.projects || MOCK_PROJECTS);
    } catch {
  ...
  const handleFiles = (key) => (e) => {
    const files = Array.from(e.target.files).map(f => ({ name: f.name, size: f.size, url: URL.createObjectURL(f), file: f }));
  ...
  const handleSubmit = async () => {
    ...
    if (!form.inspectionDate) return setError("Please select an inspection date.");
    ...
    try {
        ...
        qcChecks: [
          {
            ...
            state: form.result === "Pass" || form.result === "Pass with Conditions",
          },
        ],
        trialData: {
      ...
      setError(err.response?.data?.message || "Failed to submit QC report. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetForm = () => {
    setSubmitted(false);
    setForm({ project: "", item: "", trialType: "", result: "", inspectionDate: "", notes: "", trialFiles: [], siteImages: [] });
  };
  ...
  if (submitted) return (
    <>
      ...
      <div style={{ maxWidth: 480, margin: "80px auto", textAlign: "center" }}>
        <CheckCircle size={54} color="#34C759" strokeWidth={1.5} style={{ marginBottom: 12 }} />
        <h2 style={{ color: "#0F2854", fontSize: 22, fontWeight: 800, fontFamily: "'Syne',sans-serif", marginBottom: 8 }}>QC Report Submitted!</h2>
        <p style={{ color: "#4988C4", fontSize: 14, marginBottom: 22 }}>Trial results have been submitted for inspection review.</p>
      ...
      <div className="qc-grid">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <Card style={{ padding: "22px" }}>
            <SectionHead icon={<CheckSquare size={16} color="#BDE8F5" />} title="Recent QC Reports" subtitle="Latest submissions" />
            {recentLoading ? (
              <div style={{ display: "flex", gap: 8, alignItems: "center", color: "#4988C4", fontSize: 13 }}>
                <Loader2 size={14} style={{ animation: "spin 0.8s linear infinite" }} /> Loading…
              </div>
            ) : recentQC.length === 0 ? (
              <p style={{ color: "#4988C4", fontSize: 13, margin: 0 }}>No QC reports yet.</p>
            ) : (
              <div className="qc-report-row">
                {recentQC.map(r => {
                  const outcome = r.trialData?.trialOutcome || r.status || "Submitted";
                  const c = resultColor[outcome] || { color: "#4988C4", bg: "rgba(73,136,196,0.08)" };
                  return (
                    <div key={r._id} style={{
                      padding: "10px 14px", borderRadius: 10,
                      background: "rgba(73,136,196,0.04)", border: "1px solid rgba(73,136,196,0.1)",
                    }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, flexWrap: "wrap", gap: 4 }}>
                        <span style={{ color: "#0F2854", fontSize: 12, fontWeight: 700 }}>
                          {r.project?.name || "Project"}
                        </span>
                        <span style={{
                          background: c.bg, color: c.color,
                          padding: "1px 10px", borderRadius: 99, fontSize: 10, fontWeight: 700,
                        }}>{outcome}</span>
                      </div>
