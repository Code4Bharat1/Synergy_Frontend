...
function Label({ children, required }) {
  return (
    <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-gray-400">
      {children}
      {required && <span className="text-[#1C4D8D]"> *</span>}
    </label>
  );
}
...
function Input({
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-lg px-3.5 py-2.5 bg-white transition-all border ${
        error
          ? "border-red-400"
          : focused
            ? "border-[#1C4D8D]"
    ...
    >
      {Ic && (
        <span className={focused ? "text-[#1C4D8D]" : "text-gray-400"}>
          <Ic />
        </span>
      )}
      <input
...
function Textarea({ icon: Ic, placeholder, value, onChange, rows = 3 }) {
  ...
  return (
    <div
      className={`flex gap-3 rounded-lg px-3.5 py-2.5 bg-white transition-all border items-start ${
        focused ? "border-[#1C4D8D] shadow-sm" : "border-gray-200"
      }`}
    >
      {Ic && (
        <span
          className={`mt-0.5 ${focused ? "text-[#1C4D8D]" : "text-gray-400"}`}
        >
          <Ic />
        </span>
      )}
      <textarea
        ...
        onBlur={() => setFocused(false)}
        rows={rows}
        className="flex-1 bg-transparent text-sm outline-none resize-none placeholder-gray-300 text-extra-darkblue"
...
function MultiUserSelect({ users, selected, onChange, placeholder }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={open ? "text-[#1C4D8D]" : "text-gray-400"}>
          <Icon.Users />
        </span>
        <span
          className="flex-1 text-sm truncate"
          style={{ color: selectedUsers.length ? C.darkBlue : "#9ca3af" }}
        >
              ...
              users?.map((u) => (
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundColor: selected.includes(u._id)
                        ? C.blue
                        : "transparent",
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate text-extra-darkblue">
                      {u.name || u.email}
                    </p>
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white border border-gray-100 shadow-sm">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-xs font-bold truncate text-extra-darkblue">
      ...
      <span className="text-[10px] font-bold px-2 py-1 rounded-full flex-shrink-0 bg-[#e0eefa] text-[#1C4D8D]">
        You
      </span>
    </div>
  );
...
export default function CreateProject() {
  ...
  const marketingExecs = allUsers.filter(
    (u) => u.role === "marketingExecutive",
  );
  const engineers = allUsers.filter((u) => u.role === "engineer");
  const installationIncharges = allUsers.filter(
    (u) => u.role === "installationIncharge",
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        ...(form.assignedMarketingExecutive && {
          assignedMarketingExecutive: form.assignedMarketingExecutive,
        }),
        ...(form.assignedInstallationIncharge && {
          assignedInstallationIncharge: form.assignedInstallationIncharge,
        }),
        ...(form.assignedEngineers.length && {
  ...
  if (submitted && createdProject) {
    const assignedME = allUsers.find(
      (u) => u._id === form.assignedMarketingExecutive,
    );
    const assignedII = allUsers.find(
      (u) => u._id === form.assignedInstallationIncharge,
    );
    const assignedEng = allUsers.filter((u) =>
      form.assignedEngineers.includes(u._id),
    );
          ...
          <div className="w-full rounded-xl overflow-hidden border border-gray-100">
            {[
              ...
              >
                ...
                <span className="text-xs font-bold capitalize text-extra-darkblue">
                  {value}
                </span>
              </div>
            ))}
            <div
              ...
              style={{
                borderBottom:
                  assignedME || assignedII || assignedEng.length
                    ? "1px solid #f3f4f6"
                    : "none",
              }}
            >
            ...
            {assignedME && (
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100">
                ...
                <span className="text-xs font-bold text-extra-darkblue">
                  {assignedME.name || assignedME.email}
                </span>
              </div>
            )}
            {assignedII && (
              <div
                className="flex items-center justify-between px-4 py-2.5"
                style={{
                  borderBottom: assignedEng.length
                    ? "1px solid #f3f4f6"
                    : "none",
                }}
              >
                ...
                <span className="text-xs font-bold text-extra-darkblue">
                  {assignedII.name || assignedII.email}
                </span>
              </div>
            )}
              ...
              <div className="flex items-start justify-between px-4 py-2.5">
                ...
                <div className="flex flex-col items-end gap-0.5">
                  {assignedEng?.map((u) => (
                    <span
                      key={u._id}
                      className="text-xs font-bold text-extra-darkblue"
                    >
                      {u.name || u.email}
                    </span>
                  ))}
      ...
      <main className="p-4 md:p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <h2 className="text-base font-bold leading-tight text-extra-darkblue">
              Create New Project
            </h2>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
          ...
          <div className="px-5 md:px-7 py-4 flex items-center gap-3 border-b border-gray-100 bg-gray-50">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">
                Fill in all required information to register a new project
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="sm:col-span-2">
                  ...
                  <Input
                    icon={Icon.Hash}
                    placeholder="e.g. Gulf Leisure Phase 2 Installation"
                    value={form.name}
                    onChange={set("name")}
                    error={errors.name}
                  />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <MultiUserSelect
                          ...
                          onChange={(val) =>
                            setForm((f) => ({ ...f, assignedEngineers: val }))
                          }
            ...
            <div>
              ...
              >
                ...
                <div className="flex flex-col items-center gap-2 text-center">
                  ...
                  >
                    <span style={{ color: dragOver ? C.white : C.darkBlue }}>
                      <Icon.Upload />
                    </span>
                  </div>
              ...
              {files.length > 0 && (
                <div className="mt-3 space-y-2">
                  {files?.map((f) => (
                    <div
                      key={f.name}
                      className="flex items-center gap-3 rounded-lg px-3.5 py-2.5 bg-white border border-gray-100"
                    >
                      <span style={{ color: C.blue }}>
                        <Icon.File />
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold truncate text-extra-darkblue">
                          {f.name}
                        </p>
                        <p className="text-[11px] text-gray-400">
                          {formatSize(f.size)}
          ...
          <div className="px-5 md:px-7 py-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-t border-gray-100 bg-gray-50">
            <p className="text-[11px] text-gray-400">
              Fields marked with <span style={{ color: C.blue }}>*</span> are
              required
            </p>
