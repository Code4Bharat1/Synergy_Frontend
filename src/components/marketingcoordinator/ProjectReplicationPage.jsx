...
export default function ProjectReplicationPage() {
  ...
  const filteredItems = ITEMS_POOL.filter(
    (i) =>
      i.toLowerCase().includes(itemSearch.toLowerCase()) &&
      !linkedItems.includes(i),
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-fadeUp">
        ...
        <p className="text-brand-mid text-[14px] mb-6 text-center max-w-xs">
          Project replication for <strong>{targetName}</strong> has been
          submitted with {linkedItems.length} items.
        </p>
    ...
    <div className="animate-fadeUp">
      ...
      <div className="mb-7">
        ...
        <p className="mt-1 text-[13px] text-brand-mid">
          Replicate an existing project with linked items and assigned lead
        </p>
      </div>
          ...
          return (
            <button
              ...
              className={[
                ...
                active
                  ? "bg-gradient-to-br from-brand-darkest to-brand-dark"
                  : "",
                complete
                  ? "bg-emerald-500/7 cursor-pointer"
            ...
            >
              ...
              {active && (
                <span className="text-[8px] text-brand-light/60 bg-white/15 px-2 py-0.5 rounded-full">
                  CURRENT
                </span>
              )}
            </button>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <div className="text-brand-mid text-[11px]">
                Select the existing project to replicate
              </div>
            </div>
          </div>
          ...
          {sourceProject && (
            <div className="mb-6 rounded-xl bg-brand-bg/50 p-4 flex items-center gap-4 flex-wrap">
              ...
              <div className="min-w-0">
                <div className="text-brand-darkest font-bold text-[13px]">
                  Replicating from: {sourceProject.name}
                </div>
                <div className="text-brand-mid text-[11px]">
                  📍 {sourceProject.location} · {sourceProject.id}
                </div>
              </div>
        ...
        <div className="rounded-2xl  bg-white shadow-sm p-5 sm:p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <div className="text-brand-mid text-[11px]">
                Select items to include in this replication
              </div>
            </div>
            <span className="ml-auto text-[11px] font-bold text-brand-mid">
              {linkedItems.length} selected
            </span>
          </div>
            ...
            <div>
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                {filteredItems?.map((item) => (
                  ...
                  >
                    <span className="text-brand-darkest text-[12px] font-medium">
                      {item}
                    </span>
                ...
                {filteredItems.length === 0 && (
                  <div className="text-center py-6 text-brand-mid text-[12px]">
              ...
              {linkedItems.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                  {linkedItems?.map((item, idx) => (
                    <div
                      key={item}
                      className="flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-emerald-500/20 bg-emerald-500/6"
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <span className="text-brand-darkest text-[12px] font-semibold truncate">
                          {item}
                        </span>
                      </div>
                      <button
          ...
          <div className="mt-6 flex justify-between">
            <button
              onClick={() => setStep(1)}
              className="rounded-xl border border-brand-mid/30 px-4 py-2.5 text-[13px] font-semibold text-brand-mid hover:bg-brand-mid/8 transition-colors"
            >
              ← Back
            </button>
            <button
      ...
      {step === 3 && (
        <div className="rounded-2xl  bg-white shadow-sm p-5 sm:p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <div className="font-display font-bold text-[14px] text-brand-darkest">
                Assign Installation In-charge
              </div>
              <div className="text-brand-mid text-[11px]">
                Choose the engineer who will lead this installation
              </div>
            </div>
          </div>
            ...
            {ENGINEERS?.map((eng) => (
              ...
              >
                <div className="flex items-center gap-2.5 mb-2">
                  ...
                  <div className="min-w-0">
                    <div className="text-brand-darkest text-[13px] font-bold truncate">
                      {eng.name}
                    </div>
          ...
          <div className="mb-6 rounded-xl  bg-brand-bg/50 p-4">
            <div className="text-brand-darkest font-bold text-[12px] mb-3">
              Replication Summary
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={k}>
                  <div className="text-brand-mid text-[10px] font-semibold uppercase tracking-[0.4px]">
                    {k}
                  </div>
                  <div className="text-brand-darkest text-[13px] font-bold mt-0.5 truncate">
                    {v}
                  </div>
                </div>
              ))}
