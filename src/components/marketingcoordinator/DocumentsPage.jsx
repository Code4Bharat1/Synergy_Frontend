...
export default function DocumentsPage() {
  ...
  const removeFolderFile = (key, name) =>
    setFolderUploads((u) => ({
      ...u,
      [key]: u[key].filter((f) => f.name !== name),
    }));
  ...
  const totalFiles =
    ...
    Object.values(folderUploads).flat().length;
  const requiredDone = DOC_TYPES.filter((d) => d.required).every(
    (d) => uploads[d.key].length > 0 || folderUploads[d.key].length > 0,
  );
    ...
    <div className="animate-fadeUp">
      <div className="mb-7">
        ...
        <p className="mt-1 text-[13px] text-brand-mid">
          Submit design handover, ERP confirmation, and client communication
          logs
        </p>
      </div>
          ...
          {DOC_TYPES?.map((dt) => (
            <div key={dt.key} className="rounded-2xl bg-white shadow-sm p-5">
              <div className="flex items-center gap-3 mb-4">
                <div
                  className={`w-9 h-9 rounded-[10px] ${dt.bg} flex items-center justify-center shrink-0`}
                >
                  <dt.icon size={18} className={dt.color} strokeWidth={1.5} />
                </div>
                <div>
                  <div
                    className={`font-display font-bold text-[13px] ${dt.color}`}
                  >
                    {dt.title}
                    {dt.required && (
              ...
              <label
                className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed px-4 py-6 cursor-pointer hover:bg-slate-50 transition-all"
                style={{ borderColor: "rgba(73,136,196,0.25)" }}
                onMouseEnter={(e) =>
                  (e.currentTarget.style.borderColor = "rgba(73,136,196,0.5)")
                }
                onMouseLeave={(e) =>
                  (e.currentTarget.style.borderColor = "rgba(73,136,196,0.25)")
                }
              >
                <Upload size={20} className={dt.color} strokeWidth={1.5} />
                <span className={`text-[12px] font-semibold ${dt.color}`}>
                  Click to browse files
                </span>
                <span className="text-brand-mid text-[11px]">
                  {dt.accept} accepted · drag & drop
                </span>
                <input
                  type="file"
                  accept={dt.accept}
                  multiple
                  className="hidden"
                  onChange={addFiles(dt.key)}
                />
              </label>
              <label
                className="flex items-center justify-center gap-2 rounded-xl border border-dashed px-4 py-3 cursor-pointer hover:bg-slate-50 transition-all mt-2"
                style={{ borderColor: "rgba(73,136,196,0.25)" }}
                onMouseEnter={(e) =>
                  (e.currentTarget.style.borderColor = "rgba(73,136,196,0.5)")
                }
                ...
                <div className="mt-3 space-y-2">
                  {folderUploads[dt.key]?.map((f) => (
                    <div
                      ...
                      className={`flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 ${dt.bg} border ${dt.border}`}
                    >
                      ...
                      <button
                        onClick={() => removeFolderFile(dt.key, f.name)}
                  ...
                  {uploads[dt.key]?.map((f) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button
                          onClick={() => removeFile(dt.key, f.name)}
                          className="text-red-400 hover:text-red-600 transition-colors p-1"
                        >
                          <Trash2 size={12} />
                        </button>
            ...
            <div>
              ...
              {loading && (
                <div className="mt-2 h-1.5 w-48 rounded-full bg-brand-mid/15">
                  <div
                    className="h-1.5 rounded-full bg-brand-mid transition-all duration-300"
              ...
              {docsLoading ? (
                ...
                <div className="space-y-2">
                  {existingDocs?.map((doc) => (
                    <div
                      key={doc._id}
                      className="flex items-center justify-between gap-3 rounded-xl px-3.5 py-3 bg-slate-50 border border-brand-mid/10"
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${TYPE_STYLE[doc.documentType] || TYPE_STYLE["other"]}`}
                        >
                          {doc.documentType}
                        </span>
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_STYLE[doc.status]}`}
                        >
                          {doc.status}
                        </span>
                        <a
          ...
          <div className="rounded-2xl bg-white shadow-sm p-5 lg:sticky lg:top-6">
            ...
            <div className="space-y-3">
              {DOC_TYPES?.map((dt) => {
                ...
                return (
                  <div
                    key={dt.key}
                    className={`flex items-start gap-3 rounded-xl p-3 border transition-all ${done ? "bg-emerald-500/6 border-emerald-500/25" : "bg-brand-bg/50 border-brand-mid/10"}`}
                  >
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${done ? "bg-emerald-500" : "bg-brand-mid/15"}`}
                    >
                      {done ? (
                        <Check size={12} className="text-white" />
                      ) : (
                        <dt.icon size={11} className="text-brand-mid" />
                      )}
                    </div>
                    <div className="min-w-0">
                      <div
                        className={`text-[12px] font-bold truncate ${done ? "text-emerald-600" : "text-brand-darkest"}`}
                      >
                        {dt.title}
                      </div>
            ...
            <div className="mt-5">
              <div className="flex justify-between text-[11px] font-semibold mb-1.5">
                <span className="text-brand-mid uppercase tracking-[0.4px]">
                  Completion
                </span>
                <span className="text-brand-darkest">
                  {DOC_TYPES.filter((d) => uploads[d.key].length > 0).length}/
                  {DOC_TYPES.length}
                </span>
              </div>
              <div className="h-1.5 rounded-full bg-brand-mid/12">
                <div
                  ...
                  style={{
                    width: `${(DOC_TYPES.filter((d) => uploads[d.key].length > 0).length / DOC_TYPES.length) * 100}%`,
                  }}
                />
              </div>
            </div>
            ...
            {project && (
              <div className="mt-4 pt-4 border-t border-brand-mid/10">
                <div className="text-[11px] font-semibold text-brand-mid uppercase tracking-[0.4px] mb-1">
                  Project Documents
                </div>
                <div className="font-display text-xl font-extrabold text-brand-darkest">
                  {existingDocs.length}
                </div>
                <div className="text-[10px] text-brand-mid mt-0.5">
                  total uploaded
