...
function Toast({ msg, isError }) {
  return (
    <div
      className={`fixed top-4 right-4 z-[100] flex items-center gap-2 text-white text-sm font-semibold px-4 py-3 rounded-xl shadow-xl animate-slide-in ${isError ? "bg-red-500" : "bg-[#0F2854]"}`}
    >
      {isError ? (
        <AlertCircle size={15} />
      ) : (
...
function ExpenseDetailModal({ exp, onClose }) {
  ...
  const submitterName =
    exp.submittedBy?.name || exp.createdBy?.name || exp.user?.name || "—";
  const submitterRole =
    exp.submittedBy?.role || exp.createdBy?.role || exp.user?.role || "";
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  <span
                    className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${statusStyle(exp.status)}`}
                  >
                    {exp.status || "Pending"}
                  </span>
                  <span
                    className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${catColor(exp.category)}`}
                  >
                    {exp.category}
                  </span>
                </div>
                ...
                <p className="text-xs text-blue-200 mt-0.5">
                  {exp.projectObj?.name || "Unknown Project"}
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <div>
              ...
              <div className="space-y-0 rounded-xl border border-gray-100 overflow-hidden divide-y divide-gray-50">
                <DetailRow
                  icon={Hash}
                  label="Expense ID"
                  value={exp._id?.slice(-8).toUpperCase()}
                  mono
                />
                <DetailRow
                  icon={Building2}
                  label="Project"
                  value={exp.projectObj?.name}
                />
                {exp.projectObj?.projectId && (
                  <DetailRow
                    icon={Tag}
                    label="Project ID"
                    value={exp.projectObj.projectId}
                  />
                )}
                ...
                <DetailRow
                  icon={Calendar}
                  label="Submitted On"
                  value={fmtDateTime(exp.createdAt)}
                />
                {exp.updatedAt && exp.updatedAt !== exp.createdAt && (
                  <DetailRow
                    icon={Calendar}
                    label="Last Updated"
                    value={fmtDateTime(exp.updatedAt)}
                  />
                )}
              </div>
            </div>
...
function DetailRow({ icon: Ic, label, value, mono = false }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 bg-white">
      ...
      <span
        className={`text-xs font-semibold text-[#0F2854] truncate ${mono ? "font-mono" : ""}`}
      >
        {value}
      </span>
    </div>
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            <span
              className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${catColor(exp.category)}`}
            >
              {exp.category}
            </span>
          ...
          <span
            className={`text-[11px] font-bold px-2.5 py-1 rounded-full border shrink-0 ${statusStyle(exp.status || "Pending")}`}
          >
            {exp.status || "Pending"}
          </span>
        </div>
        ...
        <div className="pt-3 border-t border-gray-50 flex items-center justify-between">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold text-[#0F2854] truncate">
              {exp.projectObj?.name || "Unknown Project"}
            </p>
      ...
      {!loading && total > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            >
              <p className={`text-xl font-bold ${s.color || "text-[#0F2854]"}`}>
                {s.value}
              </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
          ...
          <p className="text-xs text-gray-400">
            Click "Add Expense" to submit a new one.
          </p>
        </div>
      ) : (
