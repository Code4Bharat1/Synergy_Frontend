...
const api = {
    async getProject(id) {
        const res = await fetch(`${API_BASE}/projects/${id}`, { headers: authJsonHeaders() });
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || "Failed to load project");
        return data;
    },
    async getProjectFiles(projectId) {
        const res = await fetch(`${API_BASE}/project-files/${projectId}/files`, { headers: authJsonHeaders() });
        const data = await res.json();
...
const STATUS_PROGRESS = {
    ...
    "testing": 80, "completed": 100, "on-hold": 30,
};
...
const getFileIcon = (type) => {
    if (type === "image") return <ImageIcon size={18} className="text-pink-500" />;
...
export default function ProjectDetail({ params }) {
    ...
    const backPath = pathname.includes("/director/") ? "/director/project" : "/admin/project";
    ...
    return (
        <div className="space-y-6 pb-10">
            ...
            {/* ── Toast ── */}
            {toast && (
                <div className={`fixed top-4 right-4 z-[100] flex items-center gap-2 text-white text-sm font-semibold px-5 py-3 rounded-2xl shadow-2xl backdrop-blur-sm transition-all ${toast.type === "error" ? "bg-red-500/95" : "bg-emerald-500/95"}`}>
            ...
            {preview && (
                <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
                    ...
                    <div className="relative max-w-5xl max-h-[90vh] w-full bg-white rounded-2xl shadow-2xl overflow-hidden"
                        onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
                            <p className="text-sm font-bold text-gray-700 truncate">{preview.fileName}</p>
                        ...
                        <div className="flex items-center justify-center bg-gray-100 p-4 overflow-auto" style={{ maxHeight: "80vh" }}>
                            {preview.fileType === "image" ? (
                                <img src={preview.url} alt={preview.fileName}
                                    className="max-w-full max-h-[75vh] object-contain rounded-lg shadow" />
                            ) : (
                                <iframe src={preview.url} className="w-full rounded-lg shadow bg-white"
                                    style={{ height: "75vh" }} title={preview.fileName} />
                            )}
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="p-6">
                    <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
                        <div className="flex items-center gap-3">
                            <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full ${statusStyle.bg} ${statusStyle.text}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <InfoItem icon={<Briefcase size={14} className="text-blue-500" />} label="Client" value={project.clientName} />
                        {project.clientContact && (
                            <InfoItem icon={<Phone size={14} className="text-green-500" />} label="Contact" value={project.clientContact} />
                        )}
                        {project.location && (
                            <InfoItem icon={<MapPin size={14} className="text-red-400" />} label="Location" value={project.location} />
                        )}
                        <InfoItem icon={<Calendar size={14} className="text-blue-500" />} label="Start Date" value={formatDate(project.startDate)} />
                        {project.endDate && (
                            <InfoItem icon={<Calendar size={14} className="text-orange-500" />} label="End Date" value={formatDate(project.endDate)} />
                        )}
                        {resolveName(project.assignedMarketingExecutive) && (
                            <InfoItem icon={<User size={14} className="text-purple-500" />} label="Marketing Exec" value={resolveName(project.assignedMarketingExecutive)} />
                        )}
                        {resolveName(project.assignedInstallationIncharge) && (
                            <InfoItem icon={<User size={14} className="text-indigo-500" />} label="Installation Incharge" value={resolveName(project.assignedInstallationIncharge)} />
                        )}
                        {project.assignedEngineers?.length > 0 && (
                            <InfoItem icon={<Users size={14} className="text-teal-500" />}
                                label="Engineers"
                                value={project.assignedEngineers.map(e => resolveName(e)).filter(Boolean).join(", ") || "—"} />
                        )}
                        {resolveName(project.createdBy) && (
                            <InfoItem icon={<User size={14} className="text-gray-500" />} label="Created By" value={resolveName(project.createdBy)} />
                        )}
                    </div>

                    {project.description && (
                        <div className="mt-5 p-4 bg-gray-50 rounded-xl">
                            <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1.5">Description</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-5 border-b border-gray-100">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 className="text-base font-bold text-[#0F2854]">Project Files</h2>
                            <p className="text-xs text-gray-400 mt-0.5">
                                {files.length} file{files.length !== 1 ? "s" : ""}
                                {pdfCount > 0 && <span className="ml-1">· {pdfCount} PDF{pdfCount !== 1 ? "s" : ""}</span>}
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-4">
                        ...
                        <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <button onClick={() => setFilterType("pdf")}
                            ...
                            <button onClick={() => setFilterType("image")}
                ...
                >
                    <div className="flex flex-col items-center justify-center py-6 text-center">
                        <Upload size={24} className={`mb-2 ${dragActive ? "text-blue-500" : "text-gray-300"}`} />
                        <p className="text-xs text-gray-400">
                            {dragActive ? "Drop files here…" : "Drag & drop files here, or click Upload button above"}
                        </p>
                        <p className="text-[10px] text-gray-300 mt-1">PDF, JPG, PNG, GIF, WebP, SVG (Max 10MB each)</p>
                    </div>
                </div>
...
function FileGridCard({ file, onPreview, onDelete }) {
    return (
        <div className="group relative bg-white border border-gray-100 rounded-xl overflow-hidden hover:border-blue-200 hover:shadow-md transition-all">
            {/* Preview area */}
            <div className="relative aspect-[4/3] bg-gray-50 flex items-center justify-center cursor-pointer overflow-hidden"
                onClick={onPreview}>
                {file.fileType === "image" ? (
...
function FileListRow({ file, onPreview, onDelete }) {
    return (
        <div className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl hover:border-blue-200 hover:shadow-sm transition-all group">
            {/* Thumbnail */}
            <div className="w-12 h-12 rounded-lg bg-gray-50 flex items-center justify-center shrink-0 overflow-hidden cursor-pointer"
                onClick={onPreview}>
                {file.fileType === "image" ? (
            ...
            <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-700 truncate">{file.fileName}</p>
                <div className="flex items-center gap-3 mt-0.5">
                    <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${file.fileType === "image" ? "bg-pink-50 text-pink-500" : "bg-red-50 text-red-500"}`}>
                        {file.fileType}
                    </span>
