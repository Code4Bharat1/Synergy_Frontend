...
const api = {
  async getAll({ date = "", showAll = false } = {}) {
    ...
    const res = await fetch(`${API_BASE}/attendance/all${qs}`, {
      headers: authHeaders(),
    });
...
function SummaryStats({ data }) {
  ...
  const present =
    data.filter((d) => d.status === "Present").length + late + halfDay;
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
      {stats?.map((s) => (
        <div
          key={s.label}
          className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-4 flex items-center gap-2.5 sm:gap-3"
        >
          <div
            className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${s.cls}`}
          >
            <s.icon size={16} />
          </div>
          <div>
            <p className="text-lg sm:text-xl font-bold text-extra-darkblue">
              {s.value}
            </p>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                {count} record{count !== 1 ? "s" : ""} will be rejected
              </p>
            </div>
          </div>
          <button
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span
            className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${s.cls}`}
          >
            <StatusIcon size={10} /> {record.status}
          </span>
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full ${ap.cls}`}
          >
            {ap.icon} {ap.label}
          </span>
        </div>
...
export default function AttendanceAdmin() {
  ...
  const loadAll = useCallback(
    async (all = showAll, date = filterDate) => {
      setFetching(true);
      try {
  ...
  const filtered = records.filter((r) => {
    const ms = [r.name, r.role, r.email].some((f) =>
  ...
  const paginated = approvalRecords.slice(
    (page - 1) * PAGE_SIZE,
    page * PAGE_SIZE,
  );
  ...
  useEffect(() => {
    setPage(1);
  }, [search, filterDept, filterStat, filterAppr, activeTab]);
  ...
  const allSelected =
    paginated.length > 0 && paginated.every((r) => selectedIds.has(r._id));
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await api.approveBulk(
        filterDate || todayISO(),
        selectedArr.length ? selectedArr : null,
      );
      showToast(
        `${res.approvedCount || selectedArr.length} records approved ✅`,
        "success",
      );
      setSelectedIds(new Set());
      loadAll();
  ...
  const handleReject = async (reason) => {
    ...
    try {
      ...
      showToast(
        `${res.rejectedCount || selectedArr.length} records rejected`,
        "info",
      );
      setSelectedIds(new Set());
  ...
  const handleExport = (mode, from, to) => {
    ...
    if (mode === "today") {
      ...
      toExport = records.filter(
        (r) => r.rawDate && r.rawDate >= weekAgo.toISOString().split("T")[0],
      );
      label = "this-week";
    } else if (mode === "month") {
      ...
      toExport = records.filter(
        (r) => r.rawDate && r.rawDate >= monthAgo.toISOString().split("T")[0],
      );
      label = "this-month";
    } else if (mode === "custom") {
      toExport = records.filter(
        (r) => r.rawDate && r.rawDate >= from && r.rawDate <= to,
      );
      label = `${from}-to-${to}`;
    }
  ...
  const viewLabel = showAll
    ? "All time"
    : filterDate
      ? new Date(filterDate + "T00:00:00").toLocaleDateString("en-GB", {
          day: "2-digit",
          month: "short",
          year: "numeric",
        })
    ...
    <div className="space-y-4 pb-6">
      ...
      {rejectModal && (
        <RejectModal
          count={
            selectedArr.length ||
            filtered.filter((r) => r.approvalStatus === "pending").length
          }
        ...
        {showFilters && (
          <div className="px-4 pb-4 space-y-3 border-t border-gray-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              <select
                value={filterAppr}
                onChange={(e) => setFilterAppr(e.target.value)}
                className="text-sm border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-blue-400 text-gray-700 bg-white"
              >
                {allAppr?.map((a) => (
      ...
      {!fetching && records.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Records appear once employees start punching in.
          </p>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated?.map((r) => {
                ...
                const ap =
                  APPROVAL_CONFIG[r.approvalStatus] || APPROVAL_CONFIG.pending;
                const StatusIcon = s.icon;
                return (
                  <tr
                    key={r.uid}
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="font-semibold text-extra-darkblue text-sm">
                            {r.name}
                          </p>
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`text-xs font-semibold px-2.5 py-1 rounded-full ${DEPT_COLORS[r.dept] || DEPT_COLORS.Other}`}
                      >
                        {r.dept}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-sm font-semibold text-emerald-600 whitespace-nowrap">
                      {r.punchIn || "–"}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full w-fit ${s.cls}`}
                      >
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`text-xs font-bold px-2.5 py-1 rounded-full w-fit ${ap.cls}`}
                      >
                        {ap.icon} {ap.label}
                      </span>
                    </td>
