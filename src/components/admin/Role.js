...
import { ChevronDown, X, CheckCircle2, HardHat, ShieldCheck, MessageSquareWarning, Loader } from "lucide-react";
...
function MultiSelect({ label, icon: Icon, color, selectedIds, allUsers, onChange }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        {selectedUsers.map(u => (
          <span key={u._id} className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-lightblue text-extra-blue">
            {u.name || u.email}
            <button onClick={() => onChange(selectedIds.filter(id => id !== u._id))} className="hover:text-red-500 transition-colors">
              <X size={10} />
            </button>
          </span>
        ))}
        {selectedIds.length === 0 && <span className="text-xs text-gray-300">No one assigned yet</span>}
      </div>
          ...
          {open && (
            ...
            >
              {availableUsers.map(u => (
                <button
                  key={u._id}
                  onClick={() => { onChange([...selectedIds, u._id]); setOpen(false); }}
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-blue-300 mt-0.5">
                    {a.engineers.length} engineers · {a.qc.length} QC · {a.complaints.length} handlers
                  </p>
                </div>
                <button
                ...
                >
                  {isSaving && <Loader size={12} className="animate-spin" />} Save
                </button>
              </div>
