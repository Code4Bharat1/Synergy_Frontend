  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2 items-center">
        ...
        <span
          className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_CLS[user.status] || "bg-gray-100 text-gray-400"}`}
        >
...
export default function UserManagement() {
  ...
  return (
    <div className="space-y-5">
      ...
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 flex items-center gap-2 text-white text-sm font-semibold px-4 py-3 rounded-xl shadow-lg ${toast.type === "error" ? "bg-red-500" : "bg-gray-900"}`}
        >
          {toast.type === "error" ? (
      ...
      {fetching && (
        <div className="space-y-3">
          {[1, 2, 3]?.map((i) => (
            <div
              key={i}
              className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 animate-pulse flex items-center gap-4"
            >
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered?.map((u) => (
                <tr key={u._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span
                      className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_CLS[u.status] || "bg-gray-100 text-gray-400"}`}
                    >
                      {fmtStatus(u.status)}
                    </span>
                  </td>
  ...
  return (
    <Modal title="Reset Password" onClose={onClose}>
      ...
      <Field label="New Password">
        <input
          type="password"
          className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 outline-none focus:border-blue-400 transition-colors text-gray-800 placeholder-gray-300"
          placeholder="Min 8 characters"
          value={pw}
