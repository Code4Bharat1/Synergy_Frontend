...
function CompletenessByProject({ docs }) {
  const projects = [
    ...new Set(docs?.map((d) => d.project?.name || d.project || "Unknown")),
  ];
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      {projects?.map((proj) => {
        const pd = docs.filter(
          (d) => (d.project?.name || d.project || "Unknown") === proj,
        );
        const completeCount = pd.filter((d) => d.status === "approved").length;
        const pct =
          pd.length > 0 ? Math.round((completeCount / pd.length) * 100) : 0;
        return (
          <div
            key={proj}
          ...
          >
            ...
            <div className="flex justify-between text-xs mt-2 mb-1.5">
              <span className="text-gray-400">
                {completeCount}/{pd.length} approved
              </span>
              <span
                className={`font-bold ${pct === 100 ? "text-green-600" : pct >= 50 ? "text-amber-500" : "text-red-500"}`}
              >
                {pct}%
              </span>
            </div>
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${pct}%`,
                  background:
                    pct === 100 ? "#16a34a" : pct >= 50 ? "#d97706" : "#ef4444",
                }}
              />
            </div>
          </div>
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-extra-darkblue leading-snug line-clamp-2">
            {doc.title || doc.name}
          </p>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <span
          className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full w-fit ${s.cls}`}
        >
          <StatusIcon size={10} />{" "}
      ...
      <div className="flex items-center justify-between text-xs text-gray-400 pt-1 border-t border-gray-50">
        <span>
          by {uploaderName} · {new Date(doc.createdAt).toLocaleDateString()}
        </span>
      ...
      <div className="flex gap-2">
        {doc.url && (
          <a
            ...
            className="flex-1 flex items-center justify-center py-2 rounded-lg text-xs font-semibold text-extra-blue bg-lightblue/40 hover:bg-lightblue transition-colors"
          >
...
export default function DocumentControl() {
  ...
  const ALL_PROJECTS = [
    "All Projects",
    ...new Set(
      documents?.map((d) => d.project?.name || d.project || "Unknown"),
    ),
  ];
  ...
  const filtered = documents.filter((d) => {
    ...
    const matchProject =
      filterProject === "All Projects" || pName === filterProject;
    const matchStatus =
      filterStatus === "All" || (d.status || "pending") === filterStatus;
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">
          View and verify uploaded handover documents.
        </p>
      </div>

      {fetching ? (
        <div className="py-10 text-center text-sm text-gray-500 flex justify-center items-center gap-2">
          <Loader className="animate-spin text-extra-blue" size={20} /> Loading
          documents...
        </div>
      ) : (
        <>
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-xs font-semibold text-extra-darkblue max-w-[160px] truncate">
                            {doc.title || doc.name}
                          </span>
                        </div>
                      </td>
                      <td className="px-5 py-3.5 text-xs text-gray-500 whitespace-nowrap">
                        {doc.project?.name || doc.project || "Unknown"}
                      </td>
                      <td className="px-5 py-3.5">
                        <span
                          className={`text-xs font-semibold px-2.5 py-1 rounded-full ${TYPE_COLORS[doc.documentType] || TYPE_COLORS.default}`}
                        >
                          {doc.documentType}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center gap-1 text-xs">
                          {doc.url && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-1.5 rounded-lg text-gray-400 hover:text-extra-blue hover:bg-lightblue/30 transition-all"
                            >
                              <Eye size={16} />
                            </a>
                          )}
                          {statusKey === "pending" ? (
                            <button
                              ...
                              title="Approve"
                              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-gray-500 font-bold border border-gray-200 hover:text-green-600 hover:border-green-600 transition-all disabled:opacity-50"
                            >
                              {verifyingId === doc._id ? (
