...
import { CheckCircle2, XCircle, ChevronDown, Loader2, RefreshCw } from "lucide-react";
import axiosInstance from "@/lib/axios";

const DEFAULT_QC_CHECKS = [
  { id: "qc-1", category: "Safety",        item: "Site safety measures verified",                 critical: true  },
  { id: "qc-2", category: "Documentation", item: "Material specifications match approved drawings", critical: true  },
  { id: "qc-3", category: "Quality",       item: "Workmanship quality meets standard",             critical: false },
  { id: "qc-4", category: "Dimensions",    item: "As-built dimensions within tolerance",           critical: true  },
  { id: "qc-5", category: "Structural",    item: "All connections and fixings secure",             critical: true  },
  { id: "qc-6", category: "Snag",          item: "Previous snag items resolved",                  critical: false },
  { id: "qc-7", category: "Safety",        item: "Protective measures in place",                  critical: false },
  { id: "qc-8", category: "Documentation", item: "Documentation complete and signed",              critical: false },
];
...
const freshChecks = () => DEFAULT_QC_CHECKS.map(c => ({ ...c, state: null }));
    ...
    <div className="relative">
      ...
      >
        <span className="truncate">
          {loading ? "Loading projects…" : selected ? `${selected._id.slice(-6).toUpperCase()} — ${selected.name}` : "Select a project…"}
        </span>
        {loading
        ...
        <ul className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden text-sm max-h-60 overflow-y-auto">
          {projects.map(p => (
            <li key={p._id}>
              ...
              >
                <span className="font-mono text-xs text-blue-500 font-semibold mr-2">{p._id.slice(-6).toUpperCase()}</span>
                <span className="text-blue-950 font-medium">{p.name}</span>
                <span className="text-gray-400 ml-2 text-xs">{p.clientName}</span>
              </button>
            </li>
          ))}
        </ul>
...
function ProjectDetails({ proj }) {
  const incharge = proj.assignedInstallationIncharge?.name || proj.assignedEngineers?.[0]?.name || "Unassigned";
  const fields = [
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 p-4 rounded-lg bg-blue-50/40 border border-blue-100">
      {fields.map(f => (
        <div key={f.label}>
          ...
          <p className="text-sm font-semibold text-blue-950 mt-0.5">{f.value}</p>
        </div>
      ))}
      <div>
        <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Priority</p>
        <span className={`inline-block mt-1 text-xs font-semibold px-2.5 py-0.5 rounded-full ${PRIORITY_STYLES[priorityFromStatus(proj.status)]}`}>
        ...
        {checks.map((check, i) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-sm flex-1 ${isPassed ? "font-semibold" : "font-normal"}`}>
                {check.item}
                {check.critical && <span className="ml-2 text-[10px] font-bold text-red-400 uppercase">Critical</span>}
                {check.state === null && <span className="ml-2 text-[10px] font-medium text-gray-300 uppercase">Pending</span>}
              </span>
...
function DecisionPanel({ checks, hasFailed, decision, setDecision, onSubmit, submitting, isUpdate }) {
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-3">
      ...
      {pendingCount > 0 && (
        <div className="text-xs text-amber-600 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
          ⏳ {pendingCount} item{pendingCount > 1 ? "s" : ""} still pending — you can submit now or complete them later.
        </div>
      )}
      ...
      {!canApprove && (
        <p className="text-xs text-gray-400 text-center">
          {hasFailed ? "Resolve all failed items to enable approval" : "Pass at least one item to enable approval"}
        </p>
      )}
      ...
      {decision && (
        ...
        >
          {submitting
            ...
            : isUpdate ? "Update QC Report" : "Submit QC Report"
          }
        </button>
      )}
    </div>
...
function Toast({ msg, type }) {
  if (!msg) return null;
  return (
    <div className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-lg text-sm font-semibold flex items-center gap-2
...
export default function QCInspection() {
  ...
  const handleToggle = (index, value) => {
    setChecks(prev => prev.map((c, i) => i === index ? { ...c, state: c.state === value ? null : value } : c));
  };
  ...
  const handleSelectProject = async (proj) => {
    ...
    try {
      const res = await axiosInstance.get(`/report/view-qc?project=${proj._id}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (existingReport) {
        ...
        await axiosInstance.patch(`/report/update/${existingReport._id}`, payload);
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Select a project and complete the checklist</p>
      </div>

      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-1">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Select Project</p>
        <ProjectSelector
          selected={selected}
        ...
        {reportLoading && (
          <div className="flex items-center gap-2 mt-3 text-xs text-gray-400">
            <Loader2 size={13} className="animate-spin" /> Checking for existing report…
          </div>
        )}
        {reportChecked && existingReport && (
          <div className="mt-3 flex items-center justify-between p-3 rounded-lg bg-amber-50 border border-amber-100">
            <div>
              <p className="text-xs font-semibold text-amber-700">Existing QC report found</p>
              <p className="text-xs text-amber-500 mt-0.5">
                Status: <span className="font-semibold">{existingReport.status}</span> —
                Last updated: {new Date(existingReport.updatedAt).toLocaleDateString()}
              </p>
            </div>
            <button
        ...
        {reportChecked && !existingReport && selected && (
          <p className="text-xs text-green-600 mt-2">✓ No previous report — starting fresh</p>
        )}
      </div>
