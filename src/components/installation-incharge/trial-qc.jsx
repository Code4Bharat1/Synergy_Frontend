...
const QC_ITEMS = [
  {
    id: "qc1",
    category: "Structural",
    item: "All welds inspected and certified",
    critical: true,
  },
  {
    id: "qc2",
    category: "Structural",
    item: "Anchor bolt torque verified",
    critical: true,
  },
  {
    id: "qc3",
    category: "Structural",
    item: "Support frame alignment within tolerance",
    critical: false,
  },
  {
    id: "qc4",
    category: "Hydraulics",
    item: "Pump flow rate meets specification",
    critical: true,
  },
  {
    id: "qc5",
    category: "Hydraulics",
    item: "All pipe joints pressure tested",
    critical: true,
  },
  {
    id: "qc6",
    category: "Hydraulics",
    item: "Filter system operational",
    critical: false,
  },
  {
    id: "qc7",
    category: "Surface",
    item: "Gel coat applied and cured uniformly",
    critical: false,
  },
  {
    id: "qc8",
    category: "Surface",
    item: "No delamination or surface defects",
    critical: true,
  },
  {
    id: "qc9",
    category: "Safety",
    item: "Emergency stop system functional",
    critical: true,
  },
  {
    id: "qc10",
    category: "Safety",
    item: "Safety signage installed and visible",
    critical: false,
  },
  {
    id: "qc11",
    category: "Safety",
    item: "Load capacity placard affixed",
    critical: false,
  },
  {
    id: "qc12",
    category: "Electrical",
    item: "All electrical connections tested",
    critical: true,
  },
  {
    id: "qc13",
    category: "Electrical",
    item: "Control panel commissioned",
    critical: true,
  },
];
...
const TRIAL_FIELDS = [
  ...
  },
  {
    ...
    type: "text",
    placeholder: "Client rep present",
  },
  {
    id: "waterFlowRate",
    label: "Water Flow Rate",
    type: "text",
    placeholder: "e.g. 850 L/min",
  },
  {
    id: "structuralLoadTest",
    label: "Structural Load Test",
    type: "select",
    options: ["Pass", "Fail", "Pending"],
  },
  {
    id: "safetySystemTest",
    label: "Safety System Test",
    type: "select",
    options: ["Pass", "Fail", "Pending"],
  },
  {
    ...
    type: "select",
    options: ["Satisfied", "Minor Issues", "Unsatisfied"],
  },
  {
    id: "trialOutcome",
    label: "Trial Outcome",
    type: "select",
    options: ["Passed", "Conditional Pass", "Failed"],
  },
];
...
const dbQcToState = (dbArr) => {
  const map = {};
  QC_ITEMS.forEach((c) => {
    map[c.item] = { pass: false, remarks: "" };
  });
  (dbArr || []).forEach((c) => {
    map[c.item] = { pass: !!c.pass, remarks: c.remarks || "" };
  });
  return map;
};
const stateToDbQc = (map) =>
  QC_ITEMS?.map((c) => ({
    item: c.item,
    pass: !!map[c.item]?.pass,
    remarks: map[c.item]?.remarks || "",
  }));
    ...
    try {
      ...
      const list = Array.isArray(res?.data)
        ...
        : Array.isArray(res)
          ...
          : [];
      const found =
        list.find((r) => (r.project?._id || r.project) === projectId) || null;
    ...
    try {
      ...
      if (!record) {
        ...
        await apiFetch(`/trial-qc/update/${record._id}`, {
          method: "PATCH",
          body: {
            trialDate: trialDate || record.trialDate,
            trialDetails,
            remarks,
          },
        });
...
const autoSaveTrial = async () => {
  ...
  const criticalFails = QC_ITEMS.filter(
    (c) => c.critical && !qcState[c.item]?.pass,
  ).length;
  const allCritPass = QC_ITEMS.filter((c) => c.critical).every(
    (c) => qcState[c.item]?.pass,
  );
  const trialPassed = ["Passed", "Conditional Pass"].includes(
    trialDetails.trialOutcome,
  );
  ...
  const inputCls =
    "w-full px-3 py-2 border border-gray-200 rounded-xl text-xs text-blue-950 bg-white outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all font-sans";
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-blue-950 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT sidebar */}
        <div
          className={`${showDetail ? "hidden" : "flex"} lg:flex flex-col w-full lg:w-64 lg:shrink-0 bg-white border-r border-gray-100`}
        >
        ...
        >
          {!proj ? (
            ...
            <div className="max-w-4xl w-full mx-auto">
                    ...
                    {[
                      {
                        ...
                        cls: "bg-green-500/20 text-green-200",
                      },
                      {
                      ...
                      <span
                        key={s.label}
                        className={`${s.cls} text-xs font-bold px-2.5 py-1 rounded-full`}
                      >
                        {s.label}
                      </span>
                    ))}
                    {criticalFails > 0 && (
                      <span className="bg-red-500/20 text-red-200 text-xs font-bold px-2.5 py-1 rounded-full">
                        ⚠ {criticalFails} Critical Fail
                      </span>
                    )}
                    {record ? (
                      <span
                        className={`${sm.badge} text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5`}
                      >
                        <span
                          className={`${sm.dot} w-1.5 h-1.5 rounded-full`}
                        />
                        {record.status}
                      </span>
                    ) : (
                ...
                {record?.installationIncharge && (
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-xs font-bold text-blue-950">
                      {record.installationIncharge?.name || "—"}
                    </p>
                  </div>
                )}
                {record?.qcEngineer && (
                ...
                <>
                  <div className="bg-white border border-gray-100 rounded-b-2xl overflow-hidden mb-3">
                    {CATEGORIES?.map((cat, ci) => (
                      <div key={cat}>
                        <div
                          className={`px-5 py-2.5 bg-slate-50 ${ci > 0 ? "border-t border-gray-100" : ""}`}
                        >
                          <p className="text-[10px] font-bold text-blue-500 uppercase tracking-wider">
                            {cat}
                          </p>
                        </div>
                        {QC_ITEMS.filter((c) => c.category === cat)?.map(
                          (check, i, arr) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p
                                    className={`text-xs sm:text-sm mb-1.5 ${st?.pass ? "text-blue-950 font-semibold" : "text-gray-500"}`}
                                  >
                                    {check.item}
                                  </p>
                                  <input
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  {check.critical && (
                                    <span className="bg-amber-50 text-amber-700 border border-amber-200 text-[10px] font-bold px-2 py-0.5 rounded-full hidden sm:inline">
                                      Critical
                                    </span>
                                  )}
                                  <span
                                    className={`text-xs font-bold w-8 text-right ${st?.pass ? "text-green-600" : "text-red-400"}`}
                                  >
                                    {st?.pass ? "Pass" : "Fail"}
                                  </span>
                                </div>
              ...
              {/* Approval bar */}
              <div
                className={`rounded-2xl p-4 sm:p-5 flex flex-wrap items-center justify-between gap-3 border
                ${isApproved ? "bg-green-50 border-green-200" : canApprove ? "bg-white border-gray-100" : "bg-slate-50 border-gray-100"}`}
              >
                <div>
                  <p
                    className={`text-sm font-bold mb-0.5 ${isApproved ? "text-green-700" : "text-blue-950"}`}
                  >
                  ...
                  <p className="text-xs text-gray-400">
                    {isApproved
                      ...
                      : !record
                        ? "Save QC checks first to create a record."
                        : !allCritPass
                          ? "All critical QC checks must pass before approval."
                          : !trialPassed
                            ? "Trial outcome must be Passed or Conditional Pass."
            ...
            <div className="px-5 py-4">
              <div className="grid grid-cols-2 gap-3 mb-4">
                {[
                  {
                    label: "QC Passed",
                    value: `${passedCount} / ${QC_ITEMS.length}`,
                  },
                  { label: "Critical", value: "All Passed" },
                  {
                    label: "Trial Outcome",
                    value: trialDetails.trialOutcome || "—",
                  },
                  { label: "Trial Date", value: trialDate || "—" },
                ]?.map((s) => (
                  <div
                    key={s.label}
                    className="bg-slate-50 border border-gray-100 rounded-xl p-3"
                  >
