...
function Avatar({ name, index, size = "md" }) {
  ...
  return (
    <div
      className={`${sz} ${c.avatarBg} ${c.avatarText} rounded-full flex items-center justify-center font-extrabold border ${c.border} shrink-0`}
    >
      {getInitials(name)}
    </div>
  );
  ...
  const save = async (projectId) => {
    try {
      ...
      const { data } = await axios.put(
        ...
        {
          assignedEngineers: draft.engineerIds,
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between mb-1">
                    ...
                    <span
                      className={`text-[10px] font-bold ${pct >= 0.8 ? "text-red-500" : pct >= 0.6 ? "text-amber-500" : "text-blue-700"}`}
                    >
                      {load}/{capacity}
                    </span>
                  </div>
              ...
              dl === null
                ...
                : dl < 0
                  ...
                  : dl < 7
                    ? "text-amber-500"
                    : "text-green-600";
            const assignedEngs = (project.assignedEngineers || [])
              ?.map((e) =>
                typeof e === "object"
                  ? e
                  : engineers.find((eng) => eng._id === e),
              )
              .filter(Boolean);
              ...
              >
                <div className="p-4 sm:p-5">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span
                      className={`${pm.badge} text-[10px] font-semibold px-2 py-0.5 rounded-full`}
                    >
                      {priority}
                    </span>
                  ...
                  {/* Row 2: Name + location */}
                  <p className="text-base font-bold text-blue-950 mb-0.5">
                    {project.name}
                  </p>
                    ...
                    <div className="flex-1 min-w-[160px]">
                      ...
                      {isEditing ? (
                        <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                          {engineers?.map((eng, i) => {
                            const selected = draft.engineerIds.includes(
                              eng._id,
                            );
                            return (
                              <label
                              ...
                              >
                                ...
                                <div className="min-w-0">
                                  ...
                                  <p className="text-[10px] text-gray-400 truncate">
                                    {eng.role} · {liveLoad(eng._id)} active
                                  </p>
                                </div>
                              </label>
                            );
                        ...
                        <div className="flex flex-col gap-1.5">
                          {assignedEngs?.map((eng, i) => {
                            const engIdx = engineers.findIndex(
                              (e) => e._id === (eng._id || eng),
                            );
                            return (
                              <div
                    ...
                    <div className="flex-1 min-w-[120px]">
                      ...
                      {isEditing ? (
                        ...
                        <div>
                          ...
                          <p
                            className={`text-xs font-semibold mt-0.5 ${dlCls}`}
                          >
                            {dl === null
                              ? "No date"
                              : dl < 0
