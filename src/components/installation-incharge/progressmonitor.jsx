...
const LOGS = [
  {
    date: "25 Feb 2026",
    project: "PRJ-2401",
    engineer: "Sara Hassan",
    task: "Waterslide section 4 weld inspection completed",
    hours: 8,
  },
  {
    date: "25 Feb 2026",
    project: "PRJ-2389",
    engineer: "Karim Nour",
    task: "Wave Pool Panel B re-weld — 60% done",
    hours: 7,
  },
  {
    date: "25 Feb 2026",
    project: "PRJ-2435",
    engineer: "Omar Siddiq",
    task: "Final commissioning checks passed",
    hours: 6,
  },
  {
    date: "24 Feb 2026",
    project: "PRJ-2401",
    engineer: "Sara Hassan",
    task: "Load test rig preparation",
    hours: 8,
  },
  {
    date: "24 Feb 2026",
    project: "PRJ-2376",
    engineer: "Lena Weber",
    task: "Foundation survey completed",
    hours: 5,
  },
  {
    date: "24 Feb 2026",
    project: "PRJ-2422",
    engineer: "Nadia Farouq",
    task: "Pump system installation started",
    hours: 7,
  },
  {
    date: "23 Feb 2026",
    project: "PRJ-2435",
    engineer: "Omar Siddiq",
    task: "Slide bowl alignment signed off",
    hours: 8,
  },
  {
    date: "23 Feb 2026",
    project: "PRJ-2389",
    engineer: "Karim Nour",
    task: "Structural crack assessment completed",
    hours: 4,
  },
];
...
function aggregateAttendance(records) {
  ...
  records.forEach((r) => {
    ...
    if (!map[id])
      map[id] = { name, role, present: 0, absent: 0, late: 0, total: 0 };
    map[id].total += 1;
...
function Avatar({ name, size = "md" }) {
  ...
  return (
    <div
      className={`${sz} bg-blue-50 border border-blue-200 text-blue-800 rounded-full flex items-center justify-center font-extrabold shrink-0`}
    >
      {ini(name)}
    </div>
  );
...
function ProjectCard({ p }) {
  ...
  const d = p.endDate
    ? Math.ceil((new Date(p.endDate) - new Date()) / 86400000)
    : null;
  const dlCls =
    d === null
      ? "text-gray-400"
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
      <div className="flex items-start justify-between mb-2">
        <div>
          <p className="text-[11px] font-bold text-blue-500 mb-0.5">
      ...
      <div className="flex items-center gap-2 mb-3 mt-1">
        ...
        <span
          className={`text-xs font-medium ${engineerName === "Unassigned" ? "text-red-500 font-bold" : "text-gray-600"}`}
        >
          {engineerName}
        </span>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAttendance(
          aggregateAttendance(
            attRes.data.filter((r) => r.user?.role === "engineer"),
          ),
        );
      } catch (err) {
        ...
        <div className="flex items-start justify-between flex-wrap gap-3 mb-6">
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              ...
              <span
                key={p.label}
                className={`${p.cls} inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold`}
              >
            ...
            {loading ? (
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {projects?.map((p, i) => {
                    const sm =
                      STATUS_CLS[p.displayStatus] || STATUS_CLS["On Track"];
                    const d = p.endDate
                      ? Math.ceil((new Date(p.endDate) - new Date()) / 86400000)
                      : null;
                    const engineerName = getEngineerNames(p);
                    return (
                      <tr
                        key={p._id}
                        className={`border-t border-gray-50 hover:bg-blue-50/30 transition-colors cursor-pointer ${i % 2 === 1 ? "bg-slate-50/40" : ""}`}
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <Avatar name={engineerName} size="sm" />
                            <span
                              className={`text-xs font-semibold whitespace-nowrap ${engineerName === "Unassigned" ? "text-red-500 font-bold" : "text-gray-700"}`}
                            >
                              {engineerName}
                            </span>
                          </div>
                        ...
                        <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap capitalize">
                          {STATUS_PHASE_MAP[p.status] || p.status}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {p.endDate ? (
                            <>
                              ...
                              <p
                                className={`text-[10px] font-bold mt-0.5 ${d < 0 ? "text-red-500" : d < 7 ? "text-amber-500" : "text-gray-400"}`}
                              >
                                {d < 0 ? "Overdue" : `${d}d left`}
                              </p>
                            </>
                        ...
                        <td className="px-4 py-3 min-w-[160px]">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-blue-950 w-8 text-right">
                              {p.completion}%
                            </span>
                          </div>
                        </td>
                      ...
                      return (
                        <div key={a.name + i} className="px-4 py-4">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <span
                              className={`text-sm font-extrabold ${textCls}`}
                            >
                              {rate}%
                            </span>
                          </div>
