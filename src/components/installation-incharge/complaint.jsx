...
const STATUS_DISPLAY = {
  "open":        { label: "Open",        badge: "bg-amber-100 text-amber-700 border border-amber-200", dot: "bg-amber-500"  },
  "in-progress": { label: "In Progress", badge: "bg-blue-100 text-blue-700 border border-blue-200",    dot: "bg-blue-500"   },
  "resolved":    { label: "Resolved",    badge: "bg-green-100 text-green-700 border border-green-200", dot: "bg-green-500"  },
  "closed":      { label: "Closed",      badge: "bg-red-100 text-red-700 border border-red-200",       dot: "bg-red-500"    },
};
...
export default function ComplaintApproval() {
  ...
  const resolve = async () => {
    ...
    try {
      ...
      const remaining = complaints.filter(c => c._id !== active._id && isPending(c));
  ...
  // ── Derived ────────────────────────────────────────────────────────────────
  const filterMap = { all: null, pending: ["open", "in-progress"], resolved: ["resolved"], closed: ["closed"] };

  const filtered = filter === "all"
    ? complaints
    : complaints.filter(c => filterMap[filter]?.includes(c.status));

  const active = complaints.find(c => c._id === selected) || filtered[0] || null;
  ...
  const sm = active ? (STATUS_DISPLAY[active.status] || STATUS_DISPLAY["open"]) : null;
  const pm = active ? (PRIORITY_CLS[active.priority] || PRIORITY_CLS["medium"]) : null;
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-4 flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {[
            ...
            { label: `${counts.closed} Closed`,    cls: "bg-red-50 text-red-700",     dot: "bg-red-500"   },
          ].map(s => (
        ...
        <div className={`${showDetail ? "hidden" : "flex"} lg:flex flex-col w-full lg:w-72 lg:shrink-0 bg-white border-r border-gray-100`}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {filtered.map(c => {
              ...
              return (
                <div key={c._id}
                  onClick={() => { setSelected(c._id); setShowDetail(true); }}
                  ...
                  <div className="flex justify-between items-center mb-1.5">
                    <span className={`text-[10px] font-bold tracking-wide ${isActive ? "text-blue-700" : "text-gray-400"}`}>
                      {c._id.slice(-6).toUpperCase()}
                    </span>
                    <Chip label={csm.label} badgeCls={csm.badge} dotCls={csm.dot} />
                  </div>
            ...
            <div className="max-w-3xl w-full">
              ...
              <div className="mb-5">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  ...
                  <span className="text-xs text-gray-400">{active.project?.name || "No project"}</span>
                ...
                <p className="text-xs text-gray-400">
                  {active.project?.name || "—"} · Client: {active.project?.clientName || "—"}
                </p>
              </div>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden mb-4 grid grid-cols-2 sm:grid-cols-4">
                {[
                  ...
                  { label: "Assigned To",value: active.assignedTo?.name || "Unassigned"      },
                ].map((m, i) => (
                  <div key={m.label}
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4 mb-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-bold text-blue-950">Complaint Description</span>
                </div>
                ...
                <div className="mb-4 rounded-2xl p-4 flex items-start gap-3 border bg-green-50 border-green-200">
                  ...
                  <div>
                    <p className="text-xs font-bold mb-1 text-green-700">
                      Resolved by {active.assignedTo?.name || active.loggedBy?.name || "—"} · {fmtDate(active.resolvedAt || active.updatedAt)}
                    </p>
                    {active.resolutionNotes && (
                      <p className="text-xs text-gray-500 leading-relaxed">{active.resolutionNotes}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              {active.status === "closed" && (
                <div className="mb-4 rounded-2xl p-4 flex items-start gap-3 border bg-red-50 border-red-200">
                  <div className="w-8 h-8 rounded-xl shrink-0 flex items-center justify-center bg-red-100">
                    <svg width="13" height="13" fill="none" stroke="#DC2626" strokeWidth="2.5" viewBox="0 0 24 24">
              ...
              {isPending(active) && (
                <div className="flex gap-3 flex-wrap">
                  ...
                  <button onClick={() => setModal({ type: "reject" })}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-blue-950">
                    {modal.type === "approve" ? "Mark as Resolved" : "Close Complaint"}
                  </p>
                  <p className="text-xs text-gray-400 truncate max-w-[220px]">{active.title}</p>
                </div>
              </div>
            ...
            <div className="px-5 py-4">
              ...
              <div className="bg-slate-50 border border-gray-100 rounded-xl px-4 py-3 mb-4 space-y-1">
                <p className="text-xs font-bold text-blue-950">{active.title}</p>
                <p className="text-xs text-gray-400">{active.project?.name || "No project"} · {active.priority} priority</p>
              </div>

              <label className="block text-xs font-bold text-blue-950 mb-2">
                {modal.type === "approve" ? "Resolution Notes" : "Reason for Closing"}
                <span className="font-normal text-gray-400 ml-1">— optional</span>
              </label>
              <textarea
                value={comment}
                onChange={e => setComment(e.target.value)}
                placeholder={modal.type === "approve" ? "Describe how the complaint was resolved…" : "Reason for closing this complaint…"}
                rows={3}
            ...
            <div className="px-5 pb-5 flex gap-3">
              ...
              <button onClick={resolve} disabled={actionLoading}
                ...
                {modal.type === "approve" ? "Confirm Resolved" : "Confirm Close"}
              </button>
            </div>
          </div>
        </div>
