    ...
    <div className="flex items-center justify-center gap-1 pt-3">
      ...
      {start > 1 && (
        <>
          <button
            onClick={() => onChange(1)}
            className="w-7 h-7 rounded-lg text-xs font-bold border border-gray-200 text-gray-500 hover:bg-gray-50"
          >
            1
          </button>
...
function InlineEditModal({ rec, onClose, onSaved, showToast }) {
  ...
  useEffect(() => {
    if (punchIn) setStatus(calcStatus(punchIn, punchOut));
  }, [punchIn, punchOut]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                ...
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full border ${STATUS_CONFIG[rec.status]?.cls || ""}`}
                >
                  {STATUS_CONFIG[rec.status]?.label || "Absent"}
                </span>
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full border ${APPROVAL_CONFIG[rec.approvalStatus || "pending"].cls}`}
                >
                  {APPROVAL_CONFIG[rec.approvalStatus || "pending"].icon}{" "}
                  {APPROVAL_CONFIG[rec.approvalStatus || "pending"].label}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="px-5 py-3 bg-gray-50/60 border-b border-gray-100 grid grid-cols-2 gap-2 text-xs">
          ...
          {(rec.zone || worker.zone) && (
            <div className="flex items-center gap-1.5 text-gray-500">
              <Building2 size={11} /> {rec.zone || worker.zone}
            </div>
          )}
          {fmtDate(rec.date) !== "—" && (
            <div className="flex items-center gap-1.5 text-gray-500">
              <CalendarDays size={11} /> {fmtDate(rec.date)}
            </div>
          )}
          {worker.idProof && (
          ...
          {/* Engineer + Project */}
          {(getEngineerName(rec) !== "Unknown Engineer" ||
            getProjectName(rec)) && (
        ...
        <div className="p-5 space-y-4">
          ...
          {(punchIn || punchOut) && (
            <div className="bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5 flex items-center gap-3 flex-wrap text-xs">
              ...
              {punchIn &&
                ...
                (() => {
                  ...
                  const mins = Math.round(
                    (new Date(`${today}T${punchOut}:00`) -
                      new Date(`${today}T${punchIn}:00`)) /
                      60000,
                  );
                  return mins > 0 ? (
              ...
              <span
                className={`ml-auto px-2 py-0.5 rounded-full border font-bold text-xs ${STATUS_CONFIG[status]?.cls || ""}`}
              >
        ...
        <div className="p-5 space-y-3">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {isApprove
                  ? "This finalizes attendance in the database."
                  : "Engineer will need to resubmit after corrections."}
              </p>
            </div>
          </div>
          {!isApprove && (
          ...
          <div className="flex items-start gap-2 bg-amber-50 border border-amber-100 px-3 py-2.5 rounded-xl">
            ...
            <p className="text-xs text-amber-700">
              {isApprove
                ? "You can still edit individual records after approval."
                : "Rejected records are flagged. Engineers can correct and resubmit."}
            </p>
          </div>
        </div>
        <div className="flex gap-2 px-5 pb-5">
  ...
  return (
    <div
      ...
      className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-l-4 transition-all cursor-pointer hover:shadow-md active:scale-[0.99] ${
        selected
          ? "bg-blue-50/60 border-blue-200 border-l-blue-400"
          : `bg-white border-gray-100 ${status.border}`
      }`}
    >
      <input
        type="checkbox"
      ...
      />
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${avatarBg(worker.name)}`}
      >
        {worker.name?.charAt(0)?.toUpperCase() || "?"}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {projName && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full border bg-blue-50 text-blue-600 border-blue-100 hidden sm:inline">
              🏗️ {projName}
            </span>
          )}
        </div>
          ...
          {rec.punchInTime && rec.punchOutTime && (
            <>
              ...
              <span className="text-gray-500 font-medium">
                · {fmtDuration(rec.punchInTime, rec.punchOutTime)}
              </span>
            </>
          )}
          {worker.contractor && (
      ...
      <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
        ...
        <span
          className={`text-xs font-bold px-2 py-0.5 rounded-full border ${approval.cls}`}
        >
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      >
        ...
        />
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${avatarBg(engineerName)}`}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            {pendingCount > 0 && (
              <span className="text-xs font-bold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-200">
                ⏳ {pendingCount} pending
              </span>
            )}
            {approvedCount > 0 && (
              <span className="text-xs font-bold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {projName && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full border bg-blue-50 text-blue-600 border-blue-100">
              🏗️ {projName}
            </span>
          )}
          <span
            className={`text-xs font-bold px-1.5 py-0.5 rounded-full border ${approval.cls}`}
          >
            {approval.icon} {approval.label}
          </span>
        </div>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        <span
          className={`text-xs font-bold px-2.5 py-1 rounded-full border ${status.cls}`}
        >
          {status.label}
        </span>
        <ChevronRight
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100">
          <p className="text-xs text-gray-400 font-semibold">
            Date: <span className="text-gray-700">{fmtDate(rec.date)}</span>
          </p>
          <p className="text-xs text-gray-400 font-semibold mt-0.5">
            Marked by:{" "}
            <span className="text-gray-700">{getEngineerName(rec)}</span>
          </p>
        </div>
...
export default function AdminAttendancePage() {
  ...
  const filtered = records.filter((r) => {
    ...
    const mAp =
      approvalFilter === "all" ||
      (r.approvalStatus || "pending") === approvalFilter;
    return ms && mSt && mAp;
  });
  ...
  const selectAll = (recs, select) =>
    setSelectedIds((prev) => {
      const n = new Set(prev);
      recs.forEach((r) => (select ? n.add(r._id) : n.delete(r._id)));
      return n;
    });
  const clearSelection = () => setSelectedIds(new Set());
  const pendingSelected = filtered.filter(
    (r) =>
      selectedIds.has(r._id) && (r.approvalStatus || "pending") === "pending",
  );

  const handleApproveOne = (rec) =>
    setConfirmAction({
      action: "approve",
      count: 1,
      date: dateFrom,
  ...
  const handleRecordSaved = (updated) =>
    setRecords((prev) =>
  ...
  const recPaginated = recAllSorted.slice(
    (recPage - 1) * PAGE_SIZE,
    recPage * PAGE_SIZE,
  );
  ...
  const exportCSV = () => {
    ...
    a.download = `admin_attendance_${dateFrom}_to_${dateTo}.csv`;
    a.click();
  };
    ...
    <div className="space-y-5 pb-24">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-400 ml-9">
            {fetching
              ? "Loading records…"
              : `${filtered.length} records · ${fmtDate(dateFrom)} – ${fmtDate(dateTo)}`}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-3.5">
          <div className="flex items-center justify-between mb-2 gap-2 flex-wrap">
            <span className="text-sm font-bold text-extra-darkblue">
              Approval Progress · {stats.approved} / {stats.total} approved
            </span>
            <div className="flex items-center gap-1.5 flex-wrap">
              {stats.pending > 0 && (
          ...
          <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden flex">
            <div
              ...
              style={{
                width: `${filtered.length ? (stats.approved / filtered.length) * 100 : 0}%`,
              }}
            />
            <div
              className="h-full bg-red-400 transition-all duration-700"
              style={{
                width: `${filtered.length ? (stats.rejected / filtered.length) * 100 : 0}%`,
              }}
            />
          </div>
        </div>
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="sm:col-span-3 relative">
            ...
            <input
              type="text"
              placeholder="Search worker name, engineer, contractor, project…"
              value={search}
      ...
      {!showFilters && (
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search worker, engineer, project…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div>
                  <p className="text-sm font-bold text-amber-800">
                    {stats.pending} record{stats.pending !== 1 ? "s" : ""}{" "}
            ...
            stats.rejected === 0 && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-3 flex items-center gap-3">
                ...
                <p className="text-sm font-bold text-emerald-700">
                  All records approved for this period ✅
                </p>
              </div>
            )}
          ...
          {selectedIds.size > 0 && (
            <div className="bg-extra-darkblue/5 border border-extra-darkblue/20 rounded-2xl px-4 py-3 flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <CheckCheck
                  size={15}
                  className="text-extra-darkblue shrink-0"
                />
                <span className="text-sm font-bold text-extra-darkblue">
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className="text-xs font-bold text-gray-500 px-2 py-1.5 rounded-lg hover:bg-white transition-colors"
                >
                  Clear
                </button>
                <button
                  ...
                  className="text-xs font-bold text-extra-darkblue px-2 py-1.5 rounded-lg hover:bg-white transition-colors"
                >
                ...
                <button
                  onClick={() =>
                    setConfirmAction({
                      ...
                      date: dateFrom,
                      projectId: "",
                      selectedIds: pendingSelected?.map((r) => r._id),
                    })
                  }
              ...
              {[1, 2]?.map((i) => (
                ...
                >
                  ...
                  {[1, 2, 3]?.map((j) => (
                    <div
                      key={j}
                      className="rounded-xl border border-gray-100 p-3 flex items-center gap-3 bg-gray-50 animate-pulse"
                    >
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-16 text-center">
              ...
              <p className="text-sm font-bold text-gray-500">
                {records.length > 0
                  ...
                  : "No attendance submitted for this period."}
              </p>
              {records.length > 0 && (
                <button
                  onClick={() => {
              ...
              <div className="flex items-center gap-3 px-4 py-2.5 bg-white rounded-xl border border-gray-100 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">
                    Check individual engineers below to select by group, or use
                    this to select everything
                  </p>
                </div>
                {selectedIds.size > 0 && (
                  <button
      ...
      {activeTab === "records" && (
        <SectionCard
          ...
          sub={
            fetching
              ? "Loading…"
              : `${filtered.length} records${recTotalPages > 1 ? ` · Page ${recPage} of ${recTotalPages}` : ""}`
          }
        >
          {fetching && (
            ...
            <div className="space-y-4">
              {recPagGrouped?.map(({ date, rows }) => (
                <div key={date}>
                  <div className="flex items-center gap-3 mb-2">
                    <div className="bg-extra-darkblue text-white text-xs font-bold px-3 py-1 rounded-lg whitespace-nowrap">
                      {fmtDate(date)}
                    </div>
              ...
              <p className="text-center text-xs text-gray-400 pt-1">
                ...
                <span className="font-semibold text-gray-600">
                  {(recPage - 1) * PAGE_SIZE + 1}–
                  {Math.min(recPage * PAGE_SIZE, filtered.length)}
                </span>{" "}
                of{" "}
                <span className="font-semibold text-gray-600">
                  {filtered.length}
          ...
          <div className="bg-extra-darkblue rounded-2xl shadow-2xl px-5 py-3.5 flex items-center gap-4 flex-wrap">
            <div className="flex-1 min-w-0">
              <p className="text-white text-sm font-bold">
                ⏳ {stats.pending} records still pending
              </p>
              <p className="text-white/50 text-xs mt-0.5">
                Approve or reject to finalize attendance
              </p>
            </div>
            <div className="flex gap-2 shrink-0">
              <button
