  ...
  return (
    <div className="flex items-center justify-between gap-3 flex-wrap">
      ...
      <div className="flex items-center gap-1">
        ...
        {start > 1 && (
          <>
            <button onClick={() => onChange(1)} className="w-8 h-8 rounded-lg text-xs font-bold border border-gray-200 text-gray-500 hover:bg-gray-50">1</button>
            {start > 2 && <span className="text-gray-300 text-xs px-0.5">…</span>}
          </>
        )}
        {visible.map(p => (
          <button key={p} onClick={() => onChange(p)}
            className={`w-8 h-8 rounded-lg text-xs font-bold border transition-all ${p === page ? "bg-[#0f1f3d] text-white border-[#0f1f3d]" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
            {p}
          </button>
        ))}
        {end < totalPages && (
          <>
            ...
            <button onClick={() => onChange(totalPages)} className="w-8 h-8 rounded-lg text-xs font-bold border border-gray-200 text-gray-500 hover:bg-gray-50">{totalPages}</button>
          </>
        )}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <div className="flex gap-2 px-5 pb-5">
          ...
          <button onClick={handleConfirm} disabled={loading}
            ...
            {loading ? <><RefreshCw size={13} className="animate-spin" /> Processing…</>
              : isApprove ? <><ThumbsUp size={13} /> Approve</> : <><ThumbsDown size={13} /> Reject</>}
          </button>
        </div>
      </div>
    </div>
...
function TableRow({ rec, selected, onToggleSelect, onEdit, onApproveOne, onRejectOne, idx }) {
  ...
  return (
    <tr className={`group transition-colors border-b border-gray-50 last:border-0 ${
      selected ? "bg-blue-50/60" : isApproved ? "bg-emerald-50/20" : idx % 2 === 0 ? "bg-white" : "bg-gray-50/25"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {rec.assignment?.isActive === false && (
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-400 border border-gray-200 shrink-0">Inactive</span>
              )}
            </div>
      ...
      <td className="px-4 py-4 w-[150px]">
        <div className="space-y-0.5">
          <p className="text-xs font-medium text-emerald-600">
            {rec.punchInTime ? `↑ ${fmtTime(rec.punchInTime)}` : <span className="text-gray-300">No punch in</span>}
          </p>
          <p className="text-xs text-gray-400">
            {rec.punchOutTime ? `↓ ${fmtTime(rec.punchOutTime)}` : rec.punchInTime ? <span className="text-amber-500">Still on site</span> : ""}
          </p>
        </div>
      </td>
      {/* Status */}
...
function MobileRecordCard({ rec, selected, onToggleSelect, onEdit, onApproveOne, onRejectOne }) {
  ...
  return (
    <div className={`rounded-xl border-l-4 p-3.5 transition-all ${
      ...
      {/* Top row: checkbox + avatar + name + badges */}
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
            <span className="text-sm font-bold text-[#0f1f3d] leading-tight">{worker.name || "Unknown"}</span>
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full border capitalize ${tradeCls}`}>{trade}</span>
            {rec.assignment?.isActive === false && (
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-400 border border-gray-200">Inactive</span>
            )}
            {isApproved && (
          ...
          <p className="text-xs text-gray-500 truncate">
            {getEngineerName(rec)}{projName && ` · ${projName}`}
          </p>
        </div>
        {/* Date on right */}
        <span className="text-[10px] text-gray-400 shrink-0 mt-0.5">{fmtDate(rec.date)}</span>
      </div>
      ...
      <div className="mt-2.5 grid grid-cols-2 gap-x-3 gap-y-1.5">
        ...
        <div className="flex flex-col gap-0.5">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Punch</span>
          <div className="flex items-center gap-1.5 flex-wrap">
            {rec.punchInTime
              ? <span className="text-xs font-medium text-emerald-600">↑ {fmtTime(rec.punchInTime)}</span>
              : <span className="text-xs text-gray-300">No punch in</span>}
            {rec.punchOutTime
              ? <span className="text-xs text-gray-400">↓ {fmtTime(rec.punchOutTime)}</span>
              : rec.punchInTime
                ? <span className="text-[10px] text-amber-500 font-medium animate-pulse">● On site</span>
                : null}
          </div>
          {rec.punchInTime && rec.punchOutTime && (
            <span className="text-[10px] text-gray-400">{fmtDuration(rec.punchInTime, rec.punchOutTime)}</span>
          )}
        </div>

        {/* Zone + contractor */}
        <div className="flex flex-col gap-0.5">
          {(rec.zone || worker.zone) && (
            <span className="text-xs text-gray-500">{rec.zone || worker.zone}</span>
          )}
          {worker.contractor && (
            <span className="text-xs text-gray-400 truncate"> {worker.contractor}</span>
          )}
          {worker.phone && (
      ...
      <div className="mt-2.5 flex items-center gap-1.5 flex-wrap">
        ...
        {rec.notes && (
          <span className="text-xs text-gray-400 truncate max-w-[140px]" title={rec.notes}> {rec.notes}</span>
        )}
      </div>

      {/* Action buttons */}
      <div className="flex items-center gap-2 mt-3 pt-2.5 border-t border-gray-100 justify-end">
        {isApproved ? (
          ...
          <>
            <button onClick={() => onEdit(rec)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
              <Pencil size={11} /> Edit
            </button>
            <button onClick={() => onApproveOne(rec)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold border border-emerald-200 text-emerald-600 bg-emerald-50/60 hover:bg-emerald-100 transition-colors">
              <ThumbsUp size={11} /> Approve
            </button>
          </>
        )}
        {rec.approvalStatus !== "rejected" && (
          <button onClick={() => onRejectOne(rec)}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold border border-red-200 text-red-500 bg-red-50/60 hover:bg-red-100 transition-colors">
...
export default function AdminAttendancePage() {
  ...
  useEffect(() => { setPage(1); }, [search, statusFilter, approvalFilter, engineerFilter, tradeFilter, activeFilter, sortState]);
  ...
  const filtered = records.filter(r => {
    ...
    const mActive = activeFilter === "all"
      || (activeFilter === "active" && r.assignment?.isActive !== false)
      || (activeFilter === "inactive" && r.assignment?.isActive === false);
    return ms && mSt && mAp && mEng && mTr && mActive;
  });
  ...
  const exportCSV = () => {
    const headers = ["Engineer", "Project", "Worker", "Trade", "Phone", "Contractor", "Date", "Punch In", "Punch Out", "Duration (hrs)", "Status", "Approval", "Zone", "OT (mins)", "Notes", "Worker Status"];
    const rows = sorted.map(r => [
      ...
      r.punchInTime ? new Date(r.punchInTime).toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", hour12: true }) : "",
      r.punchOutTime ? new Date(r.punchOutTime).toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", hour12: true }) : "",
      r.punchInTime && r.punchOutTime ? (Math.round((new Date(r.punchOutTime) - new Date(r.punchInTime)) / 60000) / 60).toFixed(2) : "",
      r.status || "",
      r.approvalStatus || "pending",
      r.zone || "",
      r.overtime || 0,
      r.notes || "",
      r.assignment?.isActive === false ? "Inactive" : "Active",
    ]);
    const BOM = "\uFEFF";
  ...
  const activeFilterCount = [search, statusFilter !== "all", approvalFilter !== "all", engineerFilter !== "all", tradeFilter !== "all", activeFilter !== "all"].filter(Boolean).length;
  const clearFilters = () => { setSearch(""); setStatusFilter("all"); setApprovalFilter("all"); setEngineerFilter("all"); setTradeFilter("all"); setActiveFilter("all"); };
    ...
    <div className="space-y-4 pb-28">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-[#0f1f3d]">Worker Attendance Management</h2>
          </div>
          <p className="text-xs sm:text-sm text-gray-400 ml-[42px]">
            {fetching ? "Loading records…" : `${filtered.length} records · ${fmtDate(dateFrom)} – ${fmtDate(dateTo)}`}
          </p>
        </div>

        {/* Action buttons — scrollable on mobile */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 sm:flex-wrap shrink-0">
          {/* Date range */}
          <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2 shrink-0">
            <Calendar size={12} className="text-blue-500 shrink-0" />
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
              className="bg-transparent outline-none text-blue-700 text-xs font-semibold cursor-pointer w-[88px]" />
          ...
          <button onClick={exportCSV} disabled={filtered.length === 0}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold transition-all disabled:opacity-40 active:scale-95 shrink-0">
            <Download size={13} />
          ...
          <button onClick={loadRecords} disabled={fetching}
            className="p-2 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors disabled:opacity-50 shrink-0">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          {[
            ...
            <button key={s.label}
              ...
              className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm p-2.5 sm:p-3.5 flex flex-col sm:flex-row items-center sm:items-center gap-1.5 sm:gap-3 hover:shadow-md hover:border-gray-200 transition-all text-center sm:text-left active:scale-[0.98]">
              <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 ${s.bg}`}>
                <s.icon size={14} className={s.ic} />
              </div>
              <div>
                <p className="text-base sm:text-xl font-bold text-[#0f1f3d] leading-none mb-0.5">{s.value}</p>
                <p className="text-[10px] sm:text-[11px] font-medium text-gray-400 leading-tight">{s.label}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 sm:px-5 sm:py-4">
          <div className="flex items-center justify-between mb-2.5 gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 flex-wrap text-xs">
              {stats.pending > 0 && <span className="font-bold px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-200">{stats.pending} pending</span>}
              {stats.approved > 0 && <span className="font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">{stats.approved} approved</span>}
              {stats.rejected > 0 && <span className="font-bold px-2 py-0.5 rounded-full bg-red-50 text-red-500 border border-red-200">{stats.rejected} rejected</span>}
            </div>
          </div>
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5 space-y-4">
          ...
          {/* Filter selects — 2 cols on mobile, 5 on desktop */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {[
              ...
              { label: "Worker Status",     value: activeFilter,   setter: setActiveFilter,   opts: [["all", "All Workers"], ["active", "Active Only"], ["inactive", "Inactive Only"]] },
            ].map(f => (
              <div key={f.label}>
          ...
          {activeFilterCount > 0 && (
            <div className="flex flex-wrap gap-1.5">
              ...
              {activeFilter !== "all" && <Chip label={activeFilter === "active" ? "Active Only" : "Inactive Only"} onRemove={() => setActiveFilter("all")} color="bg-gray-200 text-gray-600" />}
            </div>
          )}
        </div>
      )}

      {/* Quick search (when filters panel closed) */}
      {!showFilters && (
        <div className="relative">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300" />
      ...
      {!fetching && stats.pending > 0 && selectedIds.size === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 sm:px-5 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex gap-2 shrink-0 w-full sm:w-auto">
            <button onClick={() => setConfirmAction({ action: "reject", count: stats.pending, date: dateFrom, projectId: "", selectedIds: [] })}
              className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-red-200 bg-white text-red-500 text-xs font-bold hover:bg-red-50 active:scale-95 transition-all">
            ...
            <button onClick={() => setConfirmAction({ action: "approve", count: stats.pending, date: dateFrom, projectId: "", selectedIds: [] })}
              className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold active:scale-95 transition-all shadow-sm">
      ...
      {/* ── Bulk selection bar ── */}
      {selectedIds.size > 0 && (
        <div className="bg-[#0f1f3d] rounded-2xl px-4 py-3 sm:px-5 sm:py-3.5 flex flex-col sm:flex-row items-start sm:items-center gap-3 shadow-lg">
          <div className="flex items-center gap-2 flex-1 min-w-0">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {/* Toolbar */}
        <div className="px-4 py-3 sm:px-5 sm:py-3.5 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2 bg-gray-50/30">
          <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400 hidden sm:inline">{PAGE_SIZE}/page</span>
          </div>
        </div>
        ...
        {fetching && (
          <div className="p-4 space-y-3">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="rounded-xl bg-gray-50 animate-pulse flex items-center gap-4 px-4 py-4">
        ...
        {!fetching && paginated.length > 0 && (
          <div className="md:hidden p-3 space-y-2.5">
            ...
            <div className="flex items-center gap-2.5 px-3 py-2.5 bg-gray-50 rounded-xl border border-gray-100">
              ...
              {somePageSel && (
                <span className="ml-auto text-xs font-bold text-[#0f1f3d]">{[...selectedIds].filter(id => paginated.find(r => r._id === id)).length} selected</span>
              )}
            </div>
            {paginated.map((rec, idx) => (
        ...
        {/* Pagination */}
        {!fetching && paginated.length > 0 && (
          <div className="px-4 py-4 sm:px-5 border-t border-gray-100 bg-gray-50/30">
      ...
      {/* ── Sticky bottom bar ── */}
      {!fetching && stats.pending > 0 && selectedIds.size === 0 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-20 w-full max-w-2xl px-3 sm:px-4 pointer-events-none">
          <div className="bg-[#0f1f3d] rounded-2xl shadow-2xl px-4 py-3 sm:px-5 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 pointer-events-auto">
            <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-2 shrink-0 w-full sm:w-auto">
              <button onClick={() => setConfirmAction({ action: "reject", count: stats.pending, date: dateFrom, projectId: "", selectedIds: [] })}
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-white/20 text-white/70 text-xs font-bold hover:bg-white/10 active:scale-95 transition-all">
                <ThumbsDown size={13} /> Reject All
              </button>
              <button onClick={() => setConfirmAction({ action: "approve", count: stats.pending, date: dateFrom, projectId: "", selectedIds: [] })}
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-white text-sm font-bold active:scale-95 transition-all">
