      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100">
          <p className="text-xs text-gray-400 font-semibold">
            Attendance date:{" "}
            <span className="text-gray-700">{fmtDate(rec.date)}</span>
          </p>
        </div>
        ...
        <div className="px-5 py-4 grid grid-cols-3 gap-3 border-b border-gray-100">
          {[
            {
              label: "Punch In",
              value: fmtTime(rec.punchInTime),
              color: "text-emerald-600",
            },
            {
              label: "Punch Out",
              value: fmtTime(rec.punchOutTime),
              color: "text-red-500",
            },
            {
              label: "Duration",
              value: fmtDuration(rec.punchInTime, rec.punchOutTime),
              color: "text-gray-700",
            },
          ]?.map((t) => (
            <div
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-gray-700">
                  {fmtDate(worker.assignmentStart)}
                </p>
              </div>
            </div>
            <span className="text-gray-300 text-xs">→</span>
            <div className="flex items-center gap-1.5 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2">
              <CalendarDays
                size={12}
                className={
                  expired ? "text-red-400 shrink-0" : "text-gray-400 shrink-0"
                }
              />
              <div>
                <p className="text-[10px] text-gray-400 font-semibold">End</p>
                <p
                  className={`text-xs font-bold ${expired ? "text-red-500" : "text-gray-700"}`}
                >
            ...
            {expiry && (
              <span
                className={`text-xs font-bold px-2.5 py-1 rounded-full border ${expiry.cls}`}
              >
                {expiry.text}
              </span>
            )}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {expiry && (
            <span
              className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${expiry.cls}`}
            >
              {expiry.text}
            </span>
          )}
        ...
        <div className="flex items-center gap-2 mt-0.5 text-xs text-gray-400 flex-wrap">
          ...
          {rec.punchInTime && rec.punchOutTime && (
            <span>→ {fmtTime(rec.punchOutTime)}</span>
          )}
          {rec.punchInTime && rec.punchOutTime && (
...
export default function SiteAttendanceViewPage() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      if (!ct.includes("application/json"))
        throw new Error("Server error — check API_BASE in .env");
  ...
  useEffect(() => {
    setPage(1);
  }, [search, siteFilter, statusFilter, tradeFilter]);

  const sites = [
    ...new Set(records?.map((r) => r.worker?.site || r.site).filter(Boolean)),
  ].sort();
  const trades = [
    ...new Set(records?.map((r) => r.worker?.trade).filter(Boolean)),
  ].sort();
    ...
    <div className="space-y-5">
      ...
      {!fetching && records.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Late / Half",
              value: `Late: ${stats.late} | Half Day: ${stats.halfDay}`,
              icon: AlertCircle,
            ...
            {[
              {
                ...
                value: stats.present,
                pct: filtered.length
                  ? Math.round((stats.present / filtered.length) * 100)
                  : 0,
              ...
              },
              {
                ...
                value: stats.absent,
                pct: filtered.length
                  ? Math.round((stats.absent / filtered.length) * 100)
                  : 0,
                color: "bg-red-400",
              ...
              },
              {
                ...
                value: stats.late,
                pct: filtered.length
                  ? Math.round((stats.late / filtered.length) * 100)
                  : 0,
                color: "bg-amber-400",
              ...
              },
              {
                ...
                value: stats.halfDay,
                pct: filtered.length
                  ? Math.round((stats.halfDay / filtered.length) * 100)
                  : 0,
                color: "bg-purple-400",
              ...
              },
              {
                ...
                value: stats.onLeave,
                pct: filtered.length
                  ? Math.round((stats.onLeave / filtered.length) * 100)
                  : 0,
                color: "bg-sky-400",
              ...
              <div
                key={s.label}
                className={`rounded-xl px-3 py-2.5 ${s.light}`}
              >
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="sm:col-span-2 relative">
            ...
            <input
              type="text"
              placeholder="Search name, phone, contractor…"
              value={search}
          ...
          {[
            {
              ...
              value: siteFilter,
              set: setSiteFilter,
              opts: sites?.map((s) => ({ v: s, l: s })),
            },
            {
              label: "Status",
              value: statusFilter,
              set: setStatusFilter,
              opts: Object.entries(STATUS_CONFIG)?.map(([v, s]) => ({
                v,
                l: s.label,
              })),
            },
      ...
      <SectionCard
        ...
        sub={
          fetching
            ? "Loading…"
            : `${filtered.length} records${filtered.length !== records.length ? ` (filtered from ${records.length})` : ""}${totalPages > 1 ? ` · Page ${page} of ${totalPages}` : ""}`
        }
      >
        {fetching && (
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-sm font-semibold text-gray-400">
              {records.length > 0
                ...
                : "No attendance submitted for this period yet."}
            </p>
            {records.length > 0 && (
              <button
                onClick={() => {
