...
const TRADES = ["mason","electrician","plumber","carpenter","welder","painter","supervisor","general","other"];
...
const api = {
  ...
  async renewWorker(id, durationDays) {
    ...
    const res = await fetch(`${API_BASE}/site-workers/${id}`, {
      ...
      body: JSON.stringify({ assignmentStart: start.toISOString(), assignmentEnd: end.toISOString(), isActive: true }),
    });
    const data = await res.json();
  ...
  async lookupByIdProof(idProof) {
    ...
    const res = await fetch(`${API_BASE}/site-workers/lookup?${params}`, { headers: authHeaders() });
    const data = await res.json();
  ...
  async bulkSubmit(payload) {
    ...
    if (!ct.includes("application/json")) throw new Error(`Server error — check API_BASE (${API_BASE})`);
    const data = await res.json();
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-extra-darkblue">Add Site Worker</h3>
              <p className="text-xs text-gray-400">Appears in today's roster immediately</p>
            </div>
          </div>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 block mb-1.5">Trade</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Company" value={form.contractor}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="e.g. Block A" value={form.zone}
          ...
          <div>
            ...
            <input type="text" placeholder="XXXX-XXXX-XXXX" value={form.idProof}
        ...
        <div className="flex items-center gap-2 px-4 pb-4">
          ...
          <button onClick={handleSubmit} disabled={saving}
            ...
            {saving ? <><RefreshCw size={13} className="animate-spin"/> Adding…</> : <><UserPlus size={13}/> Add Worker</>}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" placeholder="e.g. Block A" value={record.zone || ""}
        ...
        <div className="flex items-center gap-2 px-4 pb-4">
          <button onClick={() => { onRenew(worker); onClose(); }}
    ...
    <button onClick={onClick}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5 text-xs text-gray-400 flex-wrap">
          {record.punchInTime
            ? <span className="text-emerald-600 font-medium">In {to12hr(record.punchInTime)}</span>
...
function IdLookupModal({ onClose, onRenew }) {
  ...
  const handleSearch = async () => {
    ...
    try { setResult(await api.lookupByIdProof(idInput.trim())); }
    catch (err) { setError(err.message); }
          ...
          {result && (
            <div className={`rounded-xl border-2 overflow-hidden ${result.isActive ? "border-emerald-200" : "border-amber-200"}`}>
              <div className={`px-4 py-2 flex items-center justify-between flex-wrap gap-1 text-xs font-bold ${result.isActive ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                <span>{result.isActive ? "✅ Active Worker" : "⚠️ Deactivated / Expired"}</span>
                {isExpired && !result.isActive
                  ? <span className="text-red-500">Expired {fmtDate(result.assignmentEnd)}</span>
                  : result.isActive && <span className="font-normal text-emerald-600">Ends {fmtDate(result.assignmentEnd)}</span>}
              </div>
              <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-1.5 mt-0.5 text-xs text-gray-400 flex-wrap">
          ...
          {worker.assignmentEnd && (
            <span className={isExpired ? "text-red-400 font-medium" : ""}>
              · {isExpired ? "Expired" : "Ended"} {fmtDate(worker.assignmentEnd)}
            </span>
          )}
        </div>
      </div>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function SiteAttendanceMarkPage() {
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 text-white text-sm font-semibold px-4 py-3 rounded-xl shadow-lg max-w-[calc(100vw-2rem)] ${
      ...
      <SectionCard
        icon={HardHat} iconBg="bg-orange-50" iconColor="text-orange-500"
          ...
          <>
            <button onClick={() => setShowInactive(v => !v)}
              className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                showInactive ? "bg-gray-700 text-white border-gray-700" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"
              }`}>
              {showInactive ? <><X size={10}/> Inactive</> : <>Inactive</>}
            </button>
            <button onClick={() => setShowFilters(v => !v)}
              className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                showFilters || hasActiveFilter ? "bg-extra-darkblue text-white border-extra-darkblue" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"
              }`}>
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {hasActiveFilter && (
              <button onClick={() => { setSearch(""); setFilterStatus("all"); }}
                className="text-xs font-semibold text-red-400 hover:text-red-600 px-1">Clear</button>
            )}
          </div>
        )}
          ...
          <div className="space-y-2">
            {[1,2,3,4].map(i => (
              <div key={i} className="rounded-xl border border-gray-100 p-3 animate-pulse flex items-center gap-3 bg-gray-50">
                ...
                <div className="flex-1 space-y-2"><div className="h-3 bg-gray-200 rounded w-1/4"/><div className="h-3 bg-gray-200 rounded w-1/3"/></div>
        ...
        {!fetching && workers.length === 0 && (
          <div className="py-10 text-center">
            ...
            <p className="text-sm font-semibold text-gray-500">{site ? `No active workers for "${site}"` : "No active workers found"}</p>
      ...
      {showInactive && (
        <SectionCard
          icon={Users} iconBg="bg-gray-100" iconColor="text-gray-400"
          ...
          {!fetchingInactive && paginatedInactive.length > 0 && (
            <div className="space-y-2">
              {paginatedInactive.map(w => <InactiveWorkerCard key={w._id} worker={w} onRenew={setRenewingWorker}/>)}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-4 py-3 flex items-center gap-2">
          ...
          <button onClick={handleSubmit} disabled={submitting || totalWorkers === 0}
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold transition-all active:scale-95 disabled:opacity-40 shrink-0 ${
              submitted ? "bg-emerald-500 text-white hover:bg-emerald-600" : "bg-extra-darkblue text-white hover:opacity-90"
            }`}>
            {submitting
              ? <><RefreshCw size={14} className="animate-spin"/> <span className="hidden sm:inline">Saving…</span></>
              : submitted
                ? <><CheckCheck size={14}/> <span className="hidden sm:inline">Re-submit</span></>
                : <><Send size={14}/> Submit</>}
          </button>
        </div>
      )}
